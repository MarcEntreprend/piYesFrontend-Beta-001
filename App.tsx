...
import { Info, X, CheckCircle, AlertCircle, Shield, ChevronRight, PartyPopper, Wifi, WifiOff } from 'lucide-react';
...
export const useGlobalSync = () => {
  ...
  if (!context) throw new Error("useGlobalSync must be used within a SyncProvider");
  return context;
};
...
interface SecurityContextType {
  ...
  setSecurityStatus: (status: { hasPin?: boolean; isDeviceVerified?: boolean }) => void;
...
const App: React.FC = () => {
  ...
  const [fontSize, setFontSize] = useState(localStorage.getItem('piyes-font-size') || 'default');
  const [language, setLanguage] = useState<Language>((localStorage.getItem('piyes-lang') as Language) || 'fr');
  ...
  const [securityQueue, setSecurityQueue] = useState<{ type: 'otp' | 'pin_setup' | 'pin_verify' | 'pin_intro' | 'welcome', data?: any, resolve: (pin?: string) => void } | null>(null);
  ...
  const { data: syncData, loading: syncLoading, isRefreshing, refresh } = useSync();
  ...
  const showPostSignupSecurity = () => {
    setSecurityQueue({ type: 'pin_intro', resolve: () => {
        setSecurityQueue({ type: 'welcome', resolve: () => {} });
    }});
  };

  const handleSecurityStatus = (status: { hasPin?: boolean; isDeviceVerified?: boolean }) => {
    if (status.hasPin !== undefined) setHasPin(status.hasPin);
    if (status.isDeviceVerified !== undefined) setIsDeviceVerified(status.isDeviceVerified);
  };
  ...
  const handleLogin = async (credentials: any) => {
    ...
    try {
      ...
      if (res.mfaRequired) {
        setSecurityQueue({ 
          type: 'otp', 
          data: { requestId: res.requestId, email: credentials.email, mode: 'login' },
      ...
      if (code === 'WRONG_PASSWORD' || msg.includes('Mot de passe')) {
        showToast('Mot de passe incorrect. Essayez "J´ai oublié mon mot de passe".', 'error');
      } else if (code === 'INVALID_CREDENTIALS' || msg.includes('Identifiants')) {
        showToast('Aucun compte trouvé avec ces informations. Vérifiez votre email ou numéro de téléphone.', 'error');
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener('piyes:auth_expired', handleAuthExpired);
  ...
  if (showSplash) return <Splash onComplete={() => setShowSplash(false)} isFast={!!user} />;
    ...
    <LanguageContext.Provider value={{ language, setLanguage, t }}>
      <ThemeContext.Provider value={{ theme, setTheme }}>
        ...
        <SecurityContext.Provider value={{ 
          ...
          <SyncContext.Provider value={{ syncData, syncLoading, isRefreshing, refresh }}>
            {user && isLocked && (
            <PinOverlay 
              mode="unlock" 
            ...
            {securityQueue?.type === 'otp' && (
                <OtpOverlay 
                  requestId={securityQueue.data?.requestId || securityQueue.data?.email || user?.email || "manual"} 
            ...
            {securityQueue?.type === 'pin_intro' && (
                <Modal isOpen={true} onClose={() => setSecurityQueue(null)} type="bottom-sheet">
                    <div className="p-10 space-y-8 animate-in slide-in-from-bottom duration-500">
                        <div className="w-20 h-20 theme-bubble-bg rounded-[32px] flex items-center justify-center mx-auto theme-primary-text shadow-inner">
                            <Shield size={48} strokeWidth={1.5} />
                        </div>
                        <div className="text-center space-y-3">
                            <h3 className="text-2xl font-black theme-text-main tracking-tight">{t('security_flow.pin_intro_title')}</h3>
                            <p className="text-sm theme-text-secondary leading-relaxed">{t('security_flow.pin_intro_desc')}</p>
                        </div>
                        <button 
                            onClick={() => setSecurityQueue({ type: 'pin_setup', resolve: securityQueue.resolve })}
                        ...
                        >
                            {t('security_flow.pin_setup_btn')} <ChevronRight size={20} />
                        </button>
                    </div>
                </Modal>
            )}
            ...
            {securityQueue?.type === 'pin_setup' && (
                <PinOverlay mode="setup" onSuccess={async (p) => {
                    try {
            ...
            {securityQueue?.type === 'pin_verify' && (
                <PinOverlay mode="verify" onSuccess={(p) => {
                    setSecurityQueue(null);
                    if (securityQueue.resolve) securityQueue.resolve(p);
                }} onCancel={() => setSecurityQueue(null)} onForgot={handleForgotPin} />
            )}

            {securityQueue?.type === 'welcome' && (
                <Modal isOpen={true} onClose={() => setSecurityQueue(null)} type="centered">
                    <div className="p-10 space-y-8 text-center animate-in zoom-in duration-500">
                        ...
                        <div className="space-y-2">
                            <h3 className="text-2xl font-black theme-text-main">{t('security_flow.welcome_title')}</h3>
                            <p className="text-sm theme-text-secondary">{t('security_flow.welcome_desc')}</p>
                        </div>
                        <button 
            ...
            <div className="fixed top-14 left-1/2 -translate-x-1/2 w-[90%] max-w-sm z-[200] animate-in slide-in-from-top duration-300">
                ...
                <div className={toast.type === 'error' ? 'text-red-500' : toast.type === 'success' ? 'text-green-500' : 'text-[#830AD1]'}>
                ...
                <button onClick={() => setToast(null)} className="text-white/40 dark:text-black/40 active:scale-90">
                    <X size={16} />
                </button>
                </div>
            </div>
            ...
            <div className="flex flex-col min-h-screen theme-bg max-w-md mx-auto relative shadow-2xl overflow-hidden">
                ...
                {!isOnline && (
                    <div className="w-full bg-red-600 text-white py-1.5 px-4 flex items-center justify-center gap-2 animate-in slide-in-from-top duration-500 z-[160] shrink-0">
                        ...
                        <span className="text-[10px] font-black uppercase tracking-widest">{t('common.no_internet')}</span>
                    </div>
                )}
                {showBackOnlineBar && (
                    <div className="w-full bg-green-500 text-white py-1.5 px-4 flex items-center justify-center gap-2 animate-out slide-out-to-top duration-500 delay-2000 fill-mode-forwards z-[160] shrink-0">
                        <Wifi size={14} />
                        <span className="text-[10px] font-black uppercase tracking-widest">{t('common.back_online')}</span>
                    </div>
                )}
                ...
                <Routes>
                    ...
                    <>
                        ...
                        <Route path="/request-payment" element={<RequestPayment user={user} />} />
                        <Route path="/scheduler/create" element={<SchedulerCreate />} />
                        <Route path="/settings" element={<Settings user={user} currentTheme={theme} currentFontSize={fontSize} onThemeChange={setTheme} onFontSizeChange={setFontSize} onLogout={handleLogout} />} />
                        <Route path="/security" element={<Security user={user} onLogout={handleLogout} />} />
                        <Route path="/verification" element={<Verification />} />
                        <Route path="/advanced" element={<Advanced />} />
                        <Route path="/privacy-settings" element={<PrivacySettings />} />
                        <Route path="/history" element={<History />} />
                        <Route path="/bank-history/:accountId" element={<BankHistory />} />
                        <Route path="/report" element={<Report />} />
                        <Route path="/verify-identity" element={<IdentityVerification user={user} onVerified={handleUpdateUser} />} />
                        <Route path="/transfer" element={<TransferFlow user={user} onUpdateUser={handleUpdateUser} />} />
                        <Route path="/international-transfer" element={<InternationalProviders />} />
                        <Route path="/piyes-international" element={<InternationalTransfer user={user} onUpdateUser={handleUpdateUser} />} />
                        <Route path="/deposit" element={<DepositFlow user={user} onUpdateUser={handleUpdateUser} />} />
                        <Route path="/withdraw" element={<WithdrawFlow user={user} onUpdateUser={handleUpdateUser} />} />
                        <Route path="/inter-bank-transfer" element={<InterBankTransfer user={user} onUpdateUser={handleUpdateUser} />} />
                        <Route path="/recharge" element={<MobileRecharge />} />
                        <Route path="/contacts" element={<Contacts user={user} />} />
                        <Route path="/contact-detail/:contactId" element={<ContactDetail user={user} />} />
                        <Route path="/transfer-interactions" element={<TransferInteractions />} />
                        <Route path="/receipt/:id" element={<ReceiptDetail />} />
                        <Route path="/tools" element={<FinancialTools />} />
                        <Route path="/scheduler" element={<ScheduledPayments />} />
                        <Route path="/promotions" element={<Promotions />} />
                        <Route path="/plans" element={<Plans />} />
                        <Route path="/profile" element={<Profile user={user} onUpdate={handleUpdateUser} onLogout={handleLogout} />} />
                        <Route path="/identity-hub" element={<IdentityHub user={user} onUpdate={handleUpdateUser} />} />
