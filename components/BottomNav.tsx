...
const MAIN_ROUTES = ['/', '/services', '/keys', '/settings', '/profile', '/security', '/privacy-settings', '/verification', '/help', '/support', '/about', '/terms', '/privacy'];
  ...
  return (
    <div className="fixed bottom-4 left-1/2 -translate-x-1/2 w-full max-w-md px-4 z-50 pointer-events-none flex justify-center">
      <motion.nav 
        ...
        className="theme-card-bg bg-opacity-80 dark:bg-opacity-80 backdrop-blur-xl border theme-border rounded-[32px] py-3 flex justify-between items-center shadow-[0_10px_40px_-10px_rgba(0,0,0,0.2)] pointer-events-auto overflow-hidden"
      >
        <AnimatePresence initial={false}>
          {NAV_ITEMS.map((item) => {
            ...
            const label = item.id === 'home' ? t('nav.home') :
                          item.id === 'services' ? t('nav.services') :
                          item.id === 'keys' ? t('nav.keys') : item.label;
              ...
              >
                <div 
                  ...
                  style={{ 
                      ...
                      filter: isActive ? 'drop-shadow(0 0 8px var(--primary-color))' : 'none'
                  }}
                >
                  {item.icon}
                </div>
                <span className={`text-[10px] font-bold tracking-tight transition-opacity duration-300 ${isActive ? 'opacity-100' : 'opacity-60 group-hover:opacity-100'}`}>
                  {label}
                </span>
                ...
                {isBoutique && (
                  <span className="absolute -top-1 -right-1 bg-red-500 text-white text-[7px] font-black w-4 h-4 flex items-center justify-center rounded-full border-2 border-white dark:border-gray-900 shadow-sm animate-pulse">3</span>
                )}

                {isActive && (
                  <div className="absolute -bottom-1 w-1 h-1 bg-[var(--nav-active)] rounded-full animate-in zoom-in duration-300"></div>
                )}
              </motion.button>
            );
          })}
