...
const OperationResult: React.FC<OperationResultProps> = ({ 
  ...
  const currencyName = amount > 1 ? t('currency.name_plural') : t('currency.name');
  ...
  const getMessage = () => {
    if (isSuccess) {
      if (type === 'transfer') return t('transfer.success_msg', { amount: formattedAmount, currency: currencyName });
      ...
      if (type === 'transfer') return t('transfer.failure_msg', { amount: formattedAmount, currency: currencyName, reason: errorReason });
      if (type === 'deposit') return t('deposit.failure_msg', { amount: formattedAmount, currency: currencyName, reason: errorReason });
      if (type === 'withdraw') return t('withdraw.failure_msg', { amount: formattedAmount, currency: currencyName, reason: errorReason });
  ...
  return (
    <div className="fixed inset-0 z-[100] theme-card-bg flex flex-col items-center justify-center p-8 text-center animate-in fade-in duration-500 overflow-y-auto no-scrollbar">
      <div className="flex-1 flex flex-col items-center justify-center space-y-6 w-full max-w-sm">
        <div className="relative mb-4">
          {isSuccess ? (
            ...
            <div className="rounded-full bg-red-50 dark:bg-red-900/10 p-4">
              <XCircle size={80} className="text-red-500 animate-in zoom-in duration-300" />
            </div>
          )}
        </div>
        ...
        {/* Recap Box */}
        {isSuccess && (
            <div className="w-full theme-bubble-bg rounded-[32px] p-6 my-4 border theme-border space-y-4 animate-in slide-in-from-bottom duration-700">
                <div className="flex justify-between items-center">
                    <span className="text-[10px] font-black theme-text-secondary uppercase tracking-widest">{t('interbank.total_amount')}</span>
                    <span className="text-lg font-black theme-primary-text">{formattedAmount} {t('currency.symbol')}</span>
                </div>
                <div className="h-px bg-gray-200 dark:bg-gray-800"></div>
                <div className="flex justify-between items-center">
                    <span className="text-[10px] font-black theme-text-secondary uppercase tracking-widest">{t('receipt.receiver')}</span>
                    <span className="text-sm font-black theme-text-main">{getDisplayRecipient()}</span>
                </div>
                {authCode && (
                  <>
                    <div className="h-px bg-gray-200 dark:bg-gray-800"></div>
                    <div className="flex justify-between items-center">
                        <span className="text-[10px] font-black theme-text-secondary uppercase tracking-widest">{t('intl.code_label')}</span>
                        <span className="text-sm font-black theme-primary-text">{authCode}</span>
                    </div>
                  </>
                )}
            </div>
      ...
      <div className="w-full space-y-3 max-w-sm pb-8 mt-4">
        {isSuccess && txId && (
          <Button
            fullWidth
            onClick={() => navigate(`/receipt/${txId}?type=${type}&role=${role}`)}
