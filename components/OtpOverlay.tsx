...
import { Mail, Smartphone, ChevronRight, RefreshCw, X, Loader2, ShieldCheck, HelpCircle } from 'lucide-react';
...
const OtpOverlay: React.FC<OtpOverlayProps> = ({ requestId, onSuccess, onCancel, channel = 'email', contact, mode = 'verify' }) => {
  ...
  return (
    <div className="fixed inset-y-0 left-1/2 -translate-x-1/2 w-full max-w-md z-[210] theme-card-bg flex flex-col items-center p-8 animate-in fade-in duration-300 shadow-2xl border-x theme-border">
      {onCancel && (
          <button onClick={onCancel} className="absolute top-12 left-6 p-2 theme-bubble-bg rounded-full theme-text-secondary active:scale-90 transition-transform">
              <X size={24} />
          </button>
      )}
      ...
      <div className="flex-1 flex flex-col items-center justify-center w-full max-w-xs space-y-10">
        <div className="text-center space-y-4">
            <div className="w-16 h-16 theme-bubble-bg rounded-3xl flex items-center justify-center mx-auto shadow-sm theme-primary-text">
                {channel === 'email' ? <Mail size={32} /> : <Smartphone size={32} />}
            </div>
            <div className="space-y-2">
                <h2 className="text-2xl font-black theme-text-main tracking-tight">{t('otp.title')}</h2>
                <p className="text-xs theme-text-secondary leading-relaxed">{t('otp.subtitle')}</p>
        ...
        <div className="flex gap-2 justify-center">
          {code.map((digit, i) => (
            <input
              ...
              className={`w-10 h-14 text-center text-xl font-black rounded-xl border-2 transition-all outline-none ${digit ? 'theme-primary-bg text-white border-transparent' : 'theme-bubble-bg theme-text-main theme-border focus:border-[var(--primary-color)]'}`}
              disabled={loading}
              autoFocus={i === 0}
            />
          ))}
        ...
        <div className="w-full min-h-[40px]">
            {error && (
              <div className="space-y-4 animate-in slide-in-from-top-2 duration-300">
                ...
                <div className="p-4 theme-bubble-bg rounded-[24px] border theme-border space-y-1.5 shadow-sm">
                    <div className="flex items-center gap-2">
                      <HelpCircle size={14} className="theme-primary-text" />
                      <p className="text-[11px] font-black theme-text-main">{t('otp.trouble')}</p>
                    </div>
        ...
        <div className="w-full space-y-4">
            ...
            >
                ...
                {timer > 0 ? t('otp.resend_in', { seconds: timer }) : t('otp.resend')}
            </button>
        </div>
      </div>
      ...
      <div className="pb-12 opacity-20 flex flex-col items-center gap-1">
          <ShieldCheck size={20} />
          <span className="text-[8px] font-bold uppercase tracking-widest">{t('otp.secure_connection')}</span>
      </div>
    </div>
  );
};
