...
import { Shield, Delete, Eye, EyeOff, X, AlertCircle, Clock } from 'lucide-react';
...
const PinOverlay: React.FC<PinOverlayProps> = ({ mode, onSuccess, onCancel, onFailure, onForgot, expectedPin }) => {
  ...
  return (
    <div className="fixed inset-y-0 left-1/2 -translate-x-1/2 w-full max-w-md z-[200] theme-card-bg flex flex-col items-center p-8 animate-in fade-in duration-300 shadow-2xl border-x theme-border">
      {onCancel && (
        <button onClick={onCancel} className="absolute top-12 right-6 p-2 theme-bubble-bg rounded-full theme-text-secondary active:scale-90 transition-transform" aria-label={t('common.close')}>
            <X size={24} />
        </button>
      )}
      ...
      <div className="flex-1 flex flex-col items-center justify-center w-full max-w-xs space-y-12">
        <div className="text-center space-y-4">
            <div className={`w-16 h-16 theme-bubble-bg rounded-3xl flex items-center justify-center mx-auto shadow-sm transition-colors duration-500 ${isLockedOut ? 'bg-red-50 text-red-500' : 'theme-primary-text'}`}>
                {isLockedOut ? <Clock size={32} /> : <Shield size={32} />}
            </div>
            <div className="space-y-2">
                <h2 className="text-2xl font-black theme-text-main tracking-tight">{title}</h2>
          ...
          {[...Array(4)].map((_, i) => (
            <div 
              ...
              className={`w-4 h-4 rounded-full transition-all duration-300 ${
                i < pin.length 
                ? 'theme-primary-bg scale-125 shadow-lg shadow-[var(--primary-color)]/20' 
        ...
        <div className="h-6 flex items-center justify-center">
            {error ? (
                <div className={`flex items-center gap-2 font-bold text-[11px] animate-in zoom-in px-4 py-1 rounded-full ${isLockedOut ? 'text-red-600 bg-red-50' : (attempts === 0 ? 'text-orange-600' : 'text-red-500')}`} role="alert">
                    <AlertCircle size={14} />
                    <span>{error}</span>
                </div>
            ) : (
                attempts < 3 && !isLockedOut && (
                    <p className="text-[10px] font-bold theme-text-secondary uppercase tracking-widest">{t('pin.attempts_left', { count: attempts })}</p>
                )
            )}
        </div>

        <div className={`grid grid-cols-3 gap-x-8 gap-y-6 w-full ${isLockedOut ? 'opacity-30 grayscale pointer-events-none' : ''}`}>
            {[1, 2, 3, 4, 5, 6, 7, 8, 9].map(num => (
                <button 
                  key={num} 
                  onClick={() => handleKeyPress(num.toString())}
                  className="h-16 w-16 rounded-full flex items-center justify-center text-2xl font-black theme-text-main active:theme-bubble-bg active:scale-90 transition-all focus:outline-none focus:ring-2 focus:ring-[var(--primary-color)]"
                >
                    {num}
                </button>
            ))}
        ...
        {(mode === 'unlock' || mode === 'verify' || mode === 'action') && !isLockedOut && (
            <button 
              onClick={onForgot}
