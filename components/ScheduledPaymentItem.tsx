...
import {
  ArrowUpRight, ArrowDownLeft, Check, Calendar, Clock,
  ChevronDown, ChevronUp, X, RefreshCw, Loader2, CreditCard, Trash2
} from 'lucide-react';
...
export const ScheduledPaymentItem: React.FC<ScheduledPaymentItemProps> = ({
  ...
  const [localReminders, setLocalReminders] = useState<ReminderSlot[]>(payment.reminders || []);
  ...
  const activeReminderCount = localReminders.reduce(
    (acc, r) => acc + (r.time1Active ? 1 : 0) + (r.time2Active ? 1 : 0), 0
  );
  ...
  const toggleCell = (dateStr: string, slot: 'time1' | 'time2') => {
    setLocalReminders(prev => {
      ...
      return updated.some(r => r.time1Active || r.time2Active) ? updated : prev;
    });
  };
  ...
  const toggleDay = (dateStr: string) => {
    setLocalReminders(prev => {
      ...
      const updated = prev.map(r =>
        r.date === dateStr ? { ...r, time1Active: !allActive, time2Active: !allActive } : r
      );
      return updated.some(r => r.time1Active || r.time2Active) ? updated : prev;
    });
  };
  ...
  const toggleRow = (slot: 'time1' | 'time2') => {
    setLocalReminders(prev => {
      const allActive = prev.every(r => slot === 'time1' ? r.time1Active : r.time2Active);
      const updated = prev.map(r =>
        slot === 'time1' ? { ...r, time1Active: !allActive } : { ...r, time2Active: !allActive }
      );
      return updated.some(r => r.time1Active || r.time2Active) ? updated : prev;
    });
  };
  ...
  const handleRegenerateQR = async () => {
    setLoadingQR(true);
    try {
      const { qrToken, qrExpiresAt } = await api.regenerateSchedulerQR(payment.id);
  ...
  const handlePayFromReminder = async () => {
    const noteText = payment.title
      ...
      : t('scheduler.payment_reminder_due_prefix', { date: new Date(payment.dueDate).toLocaleDateString(t('intl.locale')) });
    ...
    if (receiverUserId) {
      try {
        ...
        const contact = contacts.find(c => c.contactUserId === receiverUserId);
        if (contact) {
          // Ordre de priorité : tag > phone > email > randomKey > name
          let key = '';
          if (contact.tag) key = `@${contact.tag.replace(/^@/, '')}`;
          else if (contact.phone) key = contact.phone.startsWith('+') ? contact.phone : `+${contact.phone}`;
  ...
  const statusBadge = () => {
    switch (payment.status) {
      case 'pending':
        return <span className="bg-amber-50 text-amber-600 text-[9px] font-bold px-2 py-0.5 rounded-full border border-amber-100">{t('scheduler.list.status.pending')}</span>;
      case 'confirmed':
        return <span className="bg-blue-50 text-blue-600 text-[9px] font-bold px-2 py-0.5 rounded-full border border-blue-100">{t('scheduler.list.status.confirmed')}</span>;
      case 'paid':
        return <span className="bg-green-50 text-green-600 text-[9px] font-bold px-2 py-0.5 rounded-full border border-green-100">{t('scheduler.list.status.paid')}</span>;
      case 'cancelled':
        return <span className="bg-gray-100 text-gray-400 text-[9px] font-bold px-2 py-0.5 rounded-full border border-gray-200">{t('scheduler.list.status.cancelled')}</span>;
    }
  };
  ...
  return (
    <div
      ...
      onContextMenu={e => { e.preventDefault(); onLongPress?.(payment.id); }}
    >
      {/* ── Ligne principale ─────────────────────────────────────────────── */}
      <div
        onClick={() => isSelectionMode ? onSelect?.(payment.id) : setExpanded(!expanded)}
        className={`flex items-center gap-4 p-4 rounded-2xl border transition-all active:scale-[0.99] cursor-pointer ${
          isSelected ? 'bg-purple-50 dark:bg-purple-900/10 border-[var(--primary-color)]' :
          highlighted ? 'theme-primary-bg/5 border-[var(--primary-color)]/30' :
          'theme-card-bg shadow-sm theme-border'
        }`}
      >
        {/* Checkbox mode sélection */}
        {isSelectionMode && (
          <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center shrink-0 transition-all ${isSelected ? 'theme-primary-bg border-transparent' : 'border-gray-300'}`}>
            {isSelected && <Check size={14} className="text-white" />}
          </div>
        )}

        {/* Icône direction */}
        <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${isOutgoing ? 'bg-red-50 text-red-500' : 'bg-green-50 text-green-600'}`}>
          {isOutgoing ? <ArrowUpRight size={22} /> : <ArrowDownLeft size={22} />}
        </div>

        <div className="flex-1 space-y-1 min-w-0">
          <div className="flex justify-between items-start gap-2">
            <h4 className="font-bold theme-text-main text-sm truncate">{payment.title || t('scheduler.item.request_from', { name: payment.counterparty })}</h4>
            <span className="text-sm font-black theme-text-main shrink-0">{payment.amount.toLocaleString(t('intl.locale'))} G.</span>
          </div>
          <div className="flex justify-between items-center gap-2">
            <p className="text-[10px] theme-text-secondary truncate">
              {isOutgoing ? '→' : '←'} {payment.counterparty} · {formatDate(payment.dueDate, t('intl.locale'))}
            </p>
            {statusBadge()}
          </div>
          ...
          {activeReminderCount > 0 && payment.status !== 'cancelled' && (
            <p className="text-[9px] theme-primary-text font-bold flex items-center gap-1">
              <Clock size={9} /> {activeReminderCount > 1 ? t('scheduler.item.active_reminders_plural', { count: activeReminderCount }) : t('scheduler.item.active_reminders', { count: activeReminderCount })}
            </p>
          )}
        </div>
      ...
      {expanded && !isSelectionMode && (
        <div className="mt-2 p-4 theme-bubble-bg rounded-2xl border theme-border space-y-5 animate-in slide-in-from-top duration-200">
          ...
          <div className="flex items-center gap-2 text-xs theme-text-secondary">
            ...
            <span className="font-bold">{t('scheduler.item.due_date_full', { date: new Date(payment.dueDate).toLocaleDateString(t('intl.locale'), { day: 'numeric', month: 'long', year: 'numeric' }) })}</span>
          </div>
          ...
          {isOutgoing && payment.status === 'confirmed' && (
            ...
            >
              <CreditCard size={16} /> {t('scheduler.item.pay_now_amount', { amount: payment.amount.toLocaleString(t('intl.locale')) })}
            </button>
          )}
          ...
          {isPaid && (payment as any).paidAt && (
            <div className="flex items-center gap-2 text-xs text-green-600 font-bold">
              ...
              {t('scheduler.item.paid_at_full', { 
                date: new Date((payment as any).paidAt).toLocaleDateString(t('intl.locale'), { day: 'numeric', month: 'long', year: 'numeric' }),
                time: new Date((payment as any).paidAt).toLocaleTimeString(t('intl.locale'), { hour: '2-digit', minute: '2-digit' })
              })}
            </div>
          )}

          {/* ── Grille de rappels (receiver seulement, status confirmé seulement, pas paid) ── */}
          {isReceiver && !isPaid && payment.status !== 'cancelled' && localReminders.length > 0 && (
            <div className="space-y-3">
              <p className="text-[10px] font-black theme-text-secondary uppercase tracking-widest">{t('scheduler.reminders_grid')}</p>
              <div className="overflow-x-auto no-scrollbar">
                <table className="w-full text-center text-[10px]">
                  <thead>
                    <tr>
                      ...
                      {localReminders.map(r => (
                        <th key={r.date} onClick={() => toggleDay(r.date)}
                  ...
                  <tbody>
                    <tr>
                      <td onClick={() => toggleRow('time1')} className="text-[9px] font-black theme-text-secondary cursor-pointer active:opacity-50 text-left pr-2 py-1.5">{t('scheduler.create.time_830')}</td>
                      {localReminders.map(r => (
                        <td key={r.date} className="py-1.5 px-1">
                          <button onClick={() => toggleCell(r.date, 'time1')}
                    ...
                    <tr>
                      <td onClick={() => toggleRow('time2')} className="text-[9px] font-black theme-text-secondary cursor-pointer active:opacity-50 text-left pr-2 py-1.5">{t('scheduler.create.time_1230')}</td>
                      {localReminders.map(r => (
                        <td key={r.date} className="py-1.5 px-1">
                          <button onClick={() => toggleCell(r.date, 'time2')}
                            ...
                            {r.time2Active ? t('scheduler.create.time_1230') : '—'}
                          </button>
                        </td>
                      ))}
                    </tr>
          ...
          {qrUrl && (
            <div className="flex flex-col items-center gap-3 p-4 theme-card-bg rounded-2xl border theme-border">
              <p className="text-[10px] font-black theme-text-secondary uppercase tracking-widest">{t('scheduler.confirm_qr')}</p>
              <img src={qrUrl} alt="QR" className="w-40 h-40 rounded-xl" />
              {qrExpiry && (
                <p className="text-[9px] text-amber-500 font-bold">
                  {t('scheduler.expires_at', { time: new Date(qrExpiry).toLocaleTimeString(t('intl.locale'), { hour: '2-digit', minute: '2-digit' }) })}
                </p>
              )}
              <button
                onClick={() => {
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-black/60 backdrop-blur-sm">
          <div className="w-full max-w-sm theme-card-bg rounded-[32px] p-8 space-y-6 text-center shadow-2xl animate-in zoom-in duration-300">
            ...
            <div className="space-y-2">
              <h3 className="text-lg font-black theme-text-main">{t('scheduler.item.delete_confirm_title')}</h3>
              <p className="text-xs theme-text-secondary">
                {t('scheduler.item.delete_confirm_desc', { title: payment.title })}
              </p>
            </div>
