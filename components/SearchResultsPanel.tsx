...
import { ChevronRight, History, HelpCircle, Search, Clock, Info, X } from 'lucide-react';
      ...
      >
        ...
        >
          <div className="overflow-y-auto no-scrollbar py-4">
            {searchTerm ? (
              ...
              results.length > 0 ? (
                  ...
                  {results.map((result) => (
                    ...
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <p className="font-bold theme-text-main text-sm leading-tight group-hover:theme-primary-text transition-colors">{result.title}</p>
                          <p className="text-[9px] font-black theme-text-secondary uppercase tracking-[0.15em] opacity-50 mt-0.5">{result.category}</p>
                        </div>
                      </div>
                      <ChevronRight size={16} className="theme-text-secondary opacity-20 group-hover:opacity-100 group-hover:translate-x-1 transition-all" />
                    </button>
                  ))}
                </div>
              ) : (
                <div className="py-20 text-center space-y-4 opacity-40 px-10 animate-in fade-in duration-300">
                  <div className="w-20 h-20 theme-bubble-bg rounded-full flex items-center justify-center mx-auto mb-2">
                    <Search size={32} className="theme-text-main" strokeWidth={1.5} />
                  </div>
                  <p className="text-sm font-bold theme-text-main">Aucun résultat pour "{searchTerm}"</p>
                  <p className="text-[10px] theme-text-secondary uppercase tracking-widest leading-relaxed">Vérifiez l'orthographe ou essayez un mot-clé plus général</p>
                </div>
              )
            ) : (
              ...
              <div className="animate-in fade-in duration-500">
                {recentSearches.length > 0 && (
                  <section className="py-2">
                    <div className="px-6 mb-4 flex items-center gap-2">
                      <History size={14} className="theme-text-secondary opacity-40" />
                      <h4 className="text-[10px] font-black theme-text-secondary uppercase tracking-[0.2em]">Recherches récentes</h4>
                    </div>
                    <div className="space-y-1">
                      {recentSearches.map((query, i) => (
                        ...
                        >
                          <div className="flex items-center gap-4">
                            <Clock size={16} className="theme-text-secondary opacity-30 group-hover:theme-primary-text transition-colors" />
                            <span className="text-sm font-bold theme-text-main group-hover:theme-primary-text transition-colors">{query}</span>
                          </div>
                ...
                <section className={`py-4 ${recentSearches.length > 0 ? 'mt-4 border-t theme-border pt-6' : ''}`}>
                  <div className="px-6 mb-5 flex items-center gap-2">
                    <HelpCircle size={14} className="theme-text-secondary opacity-40" />
                    <h4 className="text-[10px] font-black theme-text-secondary uppercase tracking-[0.2em]">Questions fréquentes</h4>
                  </div>
                  <div className="space-y-2 px-2">
                    {faqs.map((faq, i) => (
                      <button 
                        ...
                        className="w-full flex flex-col p-4 hover:theme-bubble-bg transition-all text-left group rounded-[24px] border border-transparent hover:theme-border active:scale-[0.98]"
                      >
                        <div className="flex items-center gap-4">
                          ...
                          <div className="flex-1 min-w-0">
                            <span className="text-sm font-bold theme-text-main block truncate group-hover:theme-primary-text transition-colors">{faq.q}</span>
                            <p className="text-[10px] theme-text-secondary truncate opacity-60 italic font-medium leading-relaxed">"{faq.a}"</p>
                          </div>
                          <ChevronRight size={14} className="theme-text-secondary opacity-10 group-hover:opacity-100 group-hover:translate-x-1 transition-all" />
                        </div>
                      </button>
                    ))}
                  </div>
          ...
          <div className="px-6 py-4 bg-gray-50/50 dark:bg-white/5 border-t theme-border flex items-center justify-center gap-2">
              <span className="text-[9px] font-black theme-text-secondary uppercase tracking-widest opacity-40">Appuyez sur Échap pour fermer</span>
          </div>
        </div>
      </div>
    </>
