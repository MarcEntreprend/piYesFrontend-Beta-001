      ...
      <div className="p-6 space-y-8 animate-in fade-in duration-300">
        <section className="space-y-4">
          ...
          <div className="space-y-3">
            {accounts.map((acc) => (
              <div
                key={acc.id}
                className="p-4 theme-bubble-bg rounded-2xl border theme-border flex justify-between items-center"
              >
                <div>
                ...
                <div className="text-right">
                  <p className="font-bold theme-primary-text">
            ...
            {health ? (
              <div className="space-y-3">
                ...
                <div className="flex justify-between">
                  <span className="text-xs theme-text-secondary">
                    {t("advanced.health.version")}
                  </span>
                  <span className="text-xs theme-text-main font-mono">
                    {health.version}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-xs theme-text-secondary">
                    {t("advanced.health.uptime")}
                  </span>
                  <span className="text-xs theme-text-main">
                    {health.uptime}
                  </span>
                </div>
              </div>
            ) : (
              <p className="text-xs theme-text-secondary text-center py-4">
                {t("common.loading")}
              </p>
            )}
          </div>
        ...
        <section className="space-y-4">
          ...
          <div className="p-5 border theme-border rounded-2xl space-y-4">
            <p className="text-[10px] theme-text-secondary uppercase tracking-wider">
              {t("advanced.debug.label")}
            </p>
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 theme-bubble-bg border theme-border rounded-xl px-3 text-xs theme-text-main outline-none focus:border-(--primary-color)"
              />
              <button
                onClick={handleDecrypt}
                className="bg-gray-200 dark:bg-gray-700 p-3 rounded-xl active:scale-95 transition-all"
              >
                <RefreshCcw
                  size={16}
            ...
            {decryptedValue && (
              <div className="p-3 bg-black rounded-lg">
                <p className="text-[10px] font-mono text-green-500 break-all">
                  {decryptedValue}
                </p>
              </div>
            )}
