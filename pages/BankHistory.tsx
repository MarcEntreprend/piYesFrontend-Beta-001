...
import { 
  ...
  Share2, Download, Landmark
} from 'lucide-react';
import { api } from '../services/apiService';
import { Transaction, TransactionType, TransactionRole, Account } from '../shared/types';
...
const BankHistory: React.FC = () => {
  ...
  const loadTransactions = useCallback(async (isInitial = false) => {
    ...
    try {
      const data = await api.getHistory({ limit, offset: currentOffset, type: 'all', accountId });
    ...
    if (targetId && filtered.length > 0) {
        setTimeout(() => {
            ...
            if (el) {
                ...
                el.classList.add('ring-4', 'ring-[var(--primary-color)]', 'ring-opacity-20', 'rounded-2xl', 'scale-[1.02]');
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-4">
            <button onClick={() => navigate(-1)} className="p-2 -ml-2 bg-white/20 text-white rounded-full backdrop-blur-md active:scale-90 transition-transform">
              <ArrowLeft size={24} />
            </button>
            <div>
              <h1 className="text-lg font-black text-white leading-none">{account?.label}</h1>
              <p className="text-[10px] font-bold text-white/70 uppercase tracking-widest mt-1">{t('history.title')}</p>
            </div>
          </div>
          <div className="flex gap-2">
            <button className="p-2 bg-white/20 text-white rounded-full backdrop-blur-md"><Share2 size={18} /></button>
            <button 
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="theme-text-secondary text-[10px] opacity-60 mt-0.5">
                            {new Date(tx.date).toLocaleTimeString(language === 'ht' ? 'ht-HT' : 'fr-HT', { hour: '2-digit', minute: '2-digit' })}
                          </p>
                        </div>
                      </motion.div>
                    ))}
