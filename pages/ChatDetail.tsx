...
import { 
  ArrowLeft, Phone, MoreVertical, Camera, Send, Check, 
  CreditCard, ShieldCheck, X, ShoppingBag, ChevronRight,
  Info, Loader2
} from 'lucide-react';
...
const ChatDetail: React.FC = () => {
  ...
  const handleSend = async () => {
    ...
    const sentMsg = await messagingService.sendMessage(chat.id, currentMsg, 'u1'); // u1 is current user
    setChat(prev => prev ? { ...prev, messages: [...prev.messages, sentMsg] } : null);
  ...
  if (!chat) return <div className="p-8 text-center theme-text-secondary">Conversation introuvable.</div>;
    ...
    <div className="fixed inset-y-0 left-1/2 -translate-x-1/2 w-full max-w-md theme-card-bg z-[120] flex flex-col animate-in slide-in-from-right duration-400">
            ...
            <div className="flex items-center gap-3 flex-1">
                ...
                <div>
                    <div className="flex items-center gap-1">
                        <h4 className="font-black theme-text-main text-sm">{chat.counterparty.name}</h4>
                        {chat.counterparty.isVerified && <ShieldCheck size={14} className="text-blue-500" />}
                    </div>
                    <p className="text-[9px] theme-text-secondary font-black uppercase tracking-tight">Connecté</p>
                </div>
            </div>
            <button className="p-2 theme-text-secondary hover:theme-bubble-bg rounded-full"><Phone size={20} /></button>
            <button className="p-2 theme-text-secondary hover:theme-bubble-bg rounded-full"><MoreVertical size={20} /></button>
        </header>
        ...
        >
            ...
            <div className="flex-1 min-w-0">
                <p className="text-[10px] font-black theme-text-main truncate group-hover:theme-primary-text transition-colors">{chat.adTitle}</p>
                <p className="text-[11px] theme-primary-text font-black uppercase tracking-widest">{chat.adPrice.toLocaleString('fr-HT')} G.</p>
            </div>
        ...
        <main className="flex-1 overflow-y-auto p-6 space-y-6 no-scrollbar flex flex-col">
            <div className="text-center py-2">
                <span className="text-[9px] font-black theme-text-secondary theme-bubble-bg px-4 py-1.5 rounded-full uppercase tracking-[0.2em] border theme-border">
                    {chat.role === 'seller' ? `Vente : @${chat.counterparty.name.split(' ')[0]}` : `Achat : ${chat.adTitle}`}
                </span>
            </div>

            <div className="p-5 theme-bubble-bg rounded-[32px] border theme-border flex gap-4 items-start mb-4 shadow-inner">
            ...
            <div className="flex flex-col gap-5">
                {chat.messages.map((m, i) => {
                    ...
                    return (
                        <div key={m.id} className={`flex flex-col gap-1.5 max-w-[85%] ${isMe ? 'self-end items-end' : 'self-start items-start'}`}>
                            <div className={`p-4 rounded-[28px] border shadow-sm ${
                                isMe 
                            ...
                            <div className="flex items-center gap-1.5 px-2">
                                <span className="text-[8px] font-black theme-text-secondary uppercase opacity-50">{m.timestamp}</span>
                                {isMe && <Check size={10} className="text-green-500" strokeWidth={3} />}
                            </div>
                        </div>
                    );
                })}
        ...
        <footer className="p-6 theme-card-bg border-t theme-border flex flex-col gap-4 animate-in slide-in-from-bottom shadow-[0_-10px_20px_-5px_rgba(0,0,0,0.05)]">
            <div className="flex items-center gap-3">
                ...
                >
                    <input 
                        ...
                        className="w-full theme-bubble-bg py-4 pl-6 pr-14 rounded-[24px] outline-none theme-text-main text-sm focus:theme-card-bg focus:shadow-md transition-all border theme-border"
                    />
                    <button 
                        type="submit"
                        disabled={!msg.trim()}
            ...
            <div className="flex gap-2">
                <button 
                    onClick={() => navigate(`/transfer?name=${encodeURIComponent(chat.role === 'seller' ? chat.counterparty.name : chat.adTitle)}&amount=${chat.adPrice}`)}
                ...
                >
                    <CreditCard size={16} /> {chat.role === 'seller' ? 'Proposer une remise' : 'Payer avec piYès'}
                </button>
