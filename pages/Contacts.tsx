...
import { 
  ArrowLeft, UserPlus, Trash2, X, CheckCircle, 
  Send, ArrowDownLeft, Share2, 
  Phone, Mail, ShieldCheck, UserCheck, ChevronRight,
  Shield, Star, Briefcase, MapPin, RefreshCw,
  UserPlus as UserPlusIcon, UserMinus, Clock, Smartphone, AlertCircle
} from 'lucide-react';
...
const Contacts: React.FC<ContactsProps> = ({ user }) => {
  ...
  const [quickActionContact, setQuickActionContact] = useState<Contact | null>(null);
  ...
  const [newContactIsUser, setNewContactIsUser] = useState<boolean | null>(null); // null=pas encore vérifié, true=user trouvé, false=pas user
  ...
  const [pendingContactSave, setPendingContactSave] = useState<{ name: string; info: string } | null>(null);
 ...
 const fetchData = async () => {
  ...
  const handleCheckNewInfo = async () => {
    if (!newInfo.trim()) { setNewContactIsUser(null); return; }
    setCheckingNewContact(true);
    try {
      await api.resolveRecipient(newInfo.trim());
      setNewContactIsUser(true);
  ...
  const sortedAll = useMemo(() => [...contacts].sort((a, b) => a.name.localeCompare(b.name)), [contacts]);

  return (
    <div className="theme-card-bg min-h-screen pb-32 overflow-x-hidden">
      <PageHeader 
...
/>
      ...
      {/* QUICK ACTIONS MODAL */}
      <Modal isOpen={!!quickActionContact} onClose={() => setQuickActionContact(null)}>
        {quickActionContact && (
          <div className="p-8 space-y-8">
            <div className="flex items-center gap-5">
                <div className="w-16 h-16 theme-bubble-bg rounded-2xl flex items-center justify-center font-bold theme-primary-text text-xl border theme-border shadow-sm overflow-hidden">
                    {quickActionContact.avatarUrl ? (
                      <img src={quickActionContact.avatarUrl} alt={quickActionContact.name} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                    ) : getInitials(quickActionContact.name)}
                </div>
                <div>
                    <h3 className="text-xl font-bold theme-text-main">{quickActionContact.name}</h3>
                    <p className="text-xs theme-text-secondary">{quickActionContact.tag || quickActionContact.phone || quickActionContact.email}</p>
                </div>
                <button onClick={() => setQuickActionContact(null)} className="ml-auto p-2 theme-bubble-bg rounded-full theme-text-secondary active:scale-90">
                    <X size={20} />
                </button>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
                <button 
                  onClick={() => { 
                    ...
                    navigate(`/transfer?recipient=${encodeURIComponent(getPriorityKey(quickActionContact))}`); 
                  }}
                  className="flex flex-col items-center gap-3 p-6 theme-bubble-bg rounded-[32px] border theme-border active:scale-95 transition-all group"
                >
                    ...
                    <span className="text-xs font-bold theme-text-main">{t('contacts.detail.send')}</span>
                </button>

                <button 
                  disabled={getFriendshipStatus(quickActionContact.contactUserId) !== 'friends'}
                  onClick={() => { setQuickActionContact(null); navigate(`/request-payment?name=${encodeURIComponent(quickActionContact.name)}`); }}
                  className={`flex flex-col items-center gap-3 p-6 theme-bubble-bg rounded-[32px] border theme-border active:scale-95 transition-all group ${getFriendshipStatus(quickActionContact.contactUserId) !== 'friends' ? 'opacity-40 grayscale pointer-events-none' : ''}`}
                >
                    ...
                    <span className="text-xs font-bold theme-text-main">{t('contacts.detail.request')}</span>
                </button>
            </div>
            ...
            <Button
              variant="utility"
              fullWidth
              onClick={() => { setQuickActionContact(null); navigate(`/contact-detail/${quickActionContact.id}`); }}
            >
      ...
      <Modal isOpen={!!selectedContact} onClose={() => setSelectedContact(null)} type="centered">
        {selectedContact && (
          <div className="w-full max-w-sm flex flex-col">
            {/* Top Header Card */}
            <div className="theme-primary-bg p-8 pt-12 pb-16 relative">
                <button onClick={() => setSelectedContact(null)} className="absolute top-6 right-6 p-2 bg-white/20 text-white rounded-full backdrop-blur-md active:scale-90 transition-transform">
                    <X size={20} />
                </button>
                <div className="flex flex-col items-center gap-4 text-white">
                    <div className="w-24 h-24 bg-white/20 rounded-[40px] border-4 border-white/30 flex items-center justify-center text-3xl font-black shadow-xl backdrop-blur-sm overflow-hidden">
                        {selectedContact.avatarUrl ? <img src={selectedContact.avatarUrl} alt={selectedContact.name} className="w-full h-full object-cover" referrerPolicy="no-referrer" /> : getInitials(selectedContact.name)}
                    </div>
                    <div className="text-center">
                        <h3 className="text-2xl font-bold">{selectedContact.name}</h3>
                        <p className="text-xs font-bold opacity-70">{selectedContact.tag || selectedContact.phone || selectedContact.email}</p>
                    </div>
                </div>
            </div>
                ...
                <div className="space-y-4">
                    <div className="flex items-center gap-4">
                        ...
                        <div className="flex-1 border-b theme-border pb-4">
                            <p className="text-[10px] theme-text-secondary uppercase font-bold tracking-widest">{t('contacts.detail.relation')}</p>
                            <div className="flex items-center justify-between">
                                <p className="text-sm font-bold theme-text-main flex items-center gap-2">
                                    {selectedContact.type === 'company' ? t('contacts.detail.certified_company') : (getFriendshipStatus(selectedContact.contactUserId) === 'friends' ? t('contacts.detail.mutual_contact') : t('contacts.detail.simple_contact'))}
                                    {selectedContact.isVerified && <CheckCircle size={14} className="text-blue-500" />}
                                </p>
                                <button 
                                ...
                                >
                                    <Star size={20} className={selectedContact.isFavorite ? 'fill-yellow-500' : ''} />
                                </button>
                            </div>
                        </div>
                    </div>
                    ...
                    {selectedContact.contactUserId && (
                      <div className="flex items-center gap-4">
                        <div className="w-10 h-10 theme-bubble-bg rounded-xl flex items-center justify-center theme-primary-text">
                          {getFriendshipStatus(selectedContact.contactUserId) === 'friends' ? <UserCheck size={20} /> : <UserPlusIcon size={20} />}
                        </div>
                       <div className={`flex-1 border-b theme-border pb-4 rounded-xl px-2 transition-all duration-700 ${highlightFriendship ? 'bg-[var(--primary-color)]/10 ring-2 ring-[var(--primary-color)]' : ''}`}>
  <p className="text-[10px] theme-text-secondary uppercase font-bold tracking-widest">{t('contacts.detail.piyes_friends')}</p>
                          <div className="flex items-center justify-between">
                            <p className="text-sm font-bold theme-text-main">
                              {getFriendshipStatus(selectedContact.contactUserId) === 'friends' ? t('contacts.detail.you_are_friends') : 
                               getFriendshipStatus(selectedContact.contactUserId) === 'pending' ? (isRequester(selectedContact.contactUserId) ? t('contacts.detail.request_sent') : t('contacts.detail.request_received')) : 
                               t('contacts.detail.not_friends')}
                            </p>
                            <button 
                              onClick={() => handleFriendAction(selectedContact)}
                              className={`px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-2 ${
                                getFriendshipStatus(selectedContact.contactUserId) === 'friends' ? 'bg-red-500/10 text-red-500' : 
                                getFriendshipStatus(selectedContact.contactUserId) === 'pending' ? 'bg-orange-500/10 text-orange-500' : 
                                'theme-primary-bg text-white'
                              }`}
                            >
                              {getFriendshipStatus(selectedContact.contactUserId) === 'friends' ? <><UserMinus size={14} /> {t('contacts.detail.remove')}</> : 
                               getFriendshipStatus(selectedContact.contactUserId) === 'pending' ? (isRequester(selectedContact.contactUserId) ? <><X size={14} /> {t('contacts.detail.cancel')}</> : <><CheckCircle size={14} /> {t('contacts.detail.accept')}</>) : 
                               <><UserPlusIcon size={14} /> {t('contacts.detail.add')}</>}
                            </button>
                          </div>
                        </div>
                      </div>
                    ...
                    {selectedContact.type === 'company' && (
                        <>
                            <div className="flex items-center gap-4">
                                ...
                                <div className="flex-1 border-b theme-border pb-4">
                                    <p className="text-[10px] theme-text-secondary uppercase font-bold tracking-widest">{t('contacts.detail.activity')}</p>
                                    <p className="text-sm font-bold theme-text-main">{selectedContact.activityType || t('contacts.detail.commerce')}</p>
                                </div>
                            </div>
                            <div className="flex items-center gap-4">
                                <div className="w-10 h-10 theme-bubble-bg rounded-xl flex items-center justify-center theme-primary-text">
                                ...
                                <div className="flex-1 border-b theme-border pb-4">
                                    <p className="text-[10px] theme-text-secondary uppercase font-bold tracking-widest">{t('contacts.detail.address')}</p>
                                    <p className="text-sm font-bold theme-text-main">{selectedContact.address || t('contacts.detail.haiti')}</p>
                                </div>
                            </div>
                        </>
                    )}
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div className="flex-1 border-b theme-border pb-4">
                            <p className="text-[10px] theme-text-secondary uppercase font-bold tracking-widest">{t('contacts.detail.mobile')}</p>
                            <p className="text-sm font-bold theme-text-main">{selectedContact.phone || t('contacts.detail.not_specified')}</p>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div className="flex-1 border-b theme-border pb-4">
                            <p className="text-[10px] theme-text-secondary uppercase font-bold tracking-widest">{t('contacts.detail.last_interaction')}</p>
                            <p className="text-sm font-bold theme-text-main">
                              {selectedContact.lastTransactionDate ? new Date(selectedContact.lastTransactionDate).toLocaleDateString() : t('contacts.detail.none')}
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex gap-4 pt-4">
                    <Button
                      fullWidth
                      onClick={() => { 
                        navigate(`/transfer?recipient=${encodeURIComponent(getPriorityKey(selectedContact))}`); 
                        setSelectedContact(null); 
                      }}
                    >
                    ...
                    <button 
                      ...
                      onClick={() => { if(confirm(t('contacts.detail.share_confirm'))) alert(t('contacts.detail.shared_success')); }}
                    >
                        <Share2 size={24} />
                    </button>
                </div>
                ...
                <div className="flex flex-col gap-2">
                  <button 
                    onClick={() => navigate(`/transfer-interactions?contactId=${selectedContact.id}`)}
                    className="w-full theme-bubble-bg theme-text-main py-3 rounded-xl font-bold text-xs active:scale-95 transition-all"
                  >
                    {t('contacts.detail.my_interactions_with', { name: selectedContact.name.split(' ')[0] })}
                  </button>
                  <button 
      ...
      <Modal isOpen={!!showDeleteConfirm} onClose={() => setShowDeleteConfirm(null)} type="centered">
        {showDeleteConfirm && (
            <div className="p-8 text-center space-y-6">
                ...
                <div className="space-y-2">
                    <h3 className="text-xl font-bold theme-text-main">{t('contacts.detail.delete_confirm_title')}</h3>
                    <p className="text-sm theme-text-secondary">
                        {t('contacts.detail.delete_confirm_desc', { name: showDeleteConfirm.name })}
                    </p>
                </div>
        ...
        <div className="p-8 space-y-8">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-2xl font-bold theme-text-main">{t('contacts.add_title')}</h2>
            </div>
            <button onClick={() => { setShowAddModal(false); setNewContactIsUser(null); }}
          ...
          <div className="space-y-6">
            <div className="space-y-2">
              <label className="text-[10px] font-bold theme-text-secondary uppercase tracking-widest px-1">{t('contacts.name_label')}</label>
              <input type="text" placeholder={t('common.example') + " Jean Dupont"}
                value={newName} onChange={(e) => setNewName(e.target.value)}
                className="w-full theme-bubble-bg p-5 rounded-[24px] outline-none theme-text-main border theme-border focus:border-[var(--primary-color)] transition-all font-bold"
              />
            </div>

            <div className="space-y-2">
              <label className="text-[10px] font-bold theme-text-secondary uppercase tracking-widest px-1">{t('contacts.add_info_label')}</label>
              <input type="text" placeholder="@tag, email, +509..."
                value={newInfo}
                onChange={(e) => { setNewInfo(e.target.value); setNewContactIsUser(null); }}
                onBlur={handleCheckNewInfo}
                className={`w-full theme-bubble-bg p-5 rounded-[24px] outline-none theme-text-main border transition-all ${
                  newContactIsUser === true ? 'border-green-400' :
                  newContactIsUser === false ? 'border-red-400' : 'theme-border'
              ...
              {!checkingNewContact && newContactIsUser === true && (
                <p className="text-[10px] text-green-600 font-bold px-1">✓ {t('contacts.user_found')}</p>
              )}
              {!checkingNewContact && newContactIsUser === false && newInfo.trim() && (
      ...
      <Modal isOpen={showNonUserModal} onClose={() => setShowNonUserModal(false)} type="centered">
        {pendingContactSave && (
          <div className="p-8 space-y-6 text-center">
            ...
            <div className="space-y-2">
              <h3 className="text-lg font-black theme-text-main">{t('contacts.non_user_confirm_title')}</h3>
            ...
            <div className="theme-bubble-bg rounded-2xl p-4 text-left space-y-2 border theme-border">
              <p className="text-xs font-bold theme-text-secondary uppercase tracking-widest">{t('contacts.entered_info')}</p>
              <p className="text-sm font-bold theme-text-main">{pendingContactSave.name}</p>
              {pendingContactSave.info && (
                <p className="text-xs text-red-500 font-bold">{pendingContactSave.info}</p>
              )}
              <p className="text-[10px] text-amber-500 font-bold">⚠ {t('contacts.key_not_found_db')}</p>
            </div>
            <div className="flex flex-col gap-3">
              ...
              <Button
                variant="text"
                fullWidth
                onClick={() => { setShowNonUserModal(false); setPendingContactSave(null); }}
              >
