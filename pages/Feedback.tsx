...
const Feedback: React.FC = () => {
  ...
  if (sent) return (
    <div className="theme-card-bg min-h-screen flex flex-col items-center justify-center p-8 text-center space-y-6 animate-in zoom-in duration-500">
        ...
        <h2 className="text-2xl font-bold theme-text-main">Merci pour votre avis !</h2>
        <p className="theme-text-secondary text-sm">Votre retour nous aide à construire le futur de piYès.</p>
    </div>
  );
     ...
     <PageHeader
      ...
      <div className="p-8 space-y-12 flex-1 flex flex-col">
        <div className="text-center space-y-4">
            ...
            <h2 className="text-xl font-bold theme-text-main">Aidez-nous à nous améliorer</h2>
            <p className="theme-text-secondary text-sm">Comment évalueriez-vous votre expérience avec piYès ?</p>
        </div>
        ...
        <div className="flex justify-center gap-3">
            {[1, 2, 3, 4, 5].map(i => (
                ...
                >
                    <Star size={40} fill={rating >= i ? 'currentColor' : 'none'} strokeWidth={2.5} />
                </button>
            ))}
        </div>

        <div className="space-y-4 flex-1">
            <label className="text-xs font-bold theme-text-secondary uppercase tracking-widest">Commentaires (Optionnel)</label>
            <textarea 
                placeholder="Racontez-nous ce que vous aimez ou ce que nous pourrions améliorer..."
                value={comment}
                onChange={(e) => setComment(e.target.value)}
                className="w-full h-40 bg-gray-50 dark:bg-gray-800 p-6 rounded-3xl outline-none theme-text-main border theme-border focus:border-[var(--primary-color)] transition-all resize-none"
            />
        </div>
