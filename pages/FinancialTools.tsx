...
const FinancialTools: React.FC = () => {
  ...
  const CalcButton = ({
    ...
    let baseStyle =
      "h-16 rounded-full font-medium text-2xl flex items-center justify-center transition-all active:scale-90 select-none shadow-sm";
    ...
    <div className="theme-card-bg min-h-screen flex flex-col pb-6">
      ...
      <div className="flex-1 flex flex-col animate-in fade-in duration-500">
        <div className="flex flex-col justify-end py-2 px-8 text-right space-y-4">
          ...
          <div className="flex flex-col items-end">
            <p
              className={`transition-all duration-300 font-medium ${isEvaluated ? "text-6xl theme-primary-text" : "text-3xl theme-text-secondary opacity-50"}`}
            >
              {parseFloat(result).toLocaleString("fr-HT", {
        ...
        <div className="grid grid-cols-4 gap-4 p-8 pt-6">
          <CalcButton
            label={<span className="text-red-500">C</span>}
            variant="number"
            onClick={handleClear}
          />
          <CalcButton
          ...
          <div className="flex justify-between items-center">
            <div>
              <h2 className="text-2xl font-bold theme-text-main">
                {t("tools.calculator.modal_title")}
              </h2>
              <p className="text-xs theme-text-secondary mt-1">
                {t("tools.calculator.modal_sub")}
              </p>
            </div>
            <button
              onClick={() => setIsToolsModalOpen(false)}
              className="p-2 theme-bubble-bg rounded-full theme-text-secondary active:scale-90 transition-colors"
            >
              <X size={20} />
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 gap-4">
            {tools.map((tool) => (
              <button
                ...
                className="flex items-center justify-between p-5 theme-card-bg border theme-border rounded-4xl hover:theme-bubble-bg transition-all text-left active:scale-[0.98] group shadow-sm"
              >
                <div className="flex items-center gap-5">
                  ...
                  <div className="flex-1 space-y-1">
                    <div className="flex items-center gap-2">
                      ...
                      {tool.badge && (
                        <span className="text-[8px] font-bold theme-bubble-bg theme-text-secondary px-2 py-0.5 rounded-full uppercase tracking-tighter border theme-border">
                          {tool.badge}
                        </span>
                      )}
                    </div>
                    <p className="text-[10px] theme-text-secondary leading-relaxed max-w-45">
                      {tool.description}
                    </p>
                  </div>
                </div>
      ...
      {showHistoryModal && (
        <div className="fixed inset-0 bg-black/50 flex items-end z-60 animate-in fade-in duration-300">
          <div className="w-full max-w-md mx-auto theme-card-bg rounded-t-4xl p-6 space-y-6 animate-in slide-in-from-bottom duration-300">
            <div className="flex justify-between items-center">
              <h3 className="text-xl font-bold theme-text-main">
                {t("tools.calculator.history_title") ||
                ...
                history.map((entry, index) => (
                  ...
                  >
                    ...
                    <p className="text-xl font-bold theme-primary-text">
                      ={" "}
                      {parseFloat(entry.result).toLocaleString("fr-HT", {
                        maximumFractionDigits: 4,
                      })}
                    </p>
                  </button>
