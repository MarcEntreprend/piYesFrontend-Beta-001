...
import { 
  ...
  ArrowUpRight, ArrowDownLeft, Plus, ArrowDown, Globe, 
  ChevronDown, ChevronRight, CheckCircle2, LayoutList,
  Smartphone, RotateCcw, Repeat
} from 'lucide-react';
...
const History: React.FC = () => {
  ...
  const loadTransactions = useCallback(async (isInitial = false) => {
    ...
    try {
      ...
      const data = await api.getHistory({ limit, offset: currentOffset, type: 'all' });
  ...
  const filtered = useMemo(() => {
    ...
    if (activeFilter !== 'all') {
      base = allTransactions.filter(tx => {
        if (activeFilter === 'received') return tx.role === TransactionRole.RECEIVER && tx.type === TransactionType.TRANSFER;
        if (activeFilter === 'sent') return tx.role === TransactionRole.PAYER && tx.type === TransactionType.TRANSFER;
        if (activeFilter === 'deposits') return tx.type === TransactionType.DEPOSIT;
        if (activeFilter === 'withdrawals') return tx.type === TransactionType.WITHDRAW;
        if (activeFilter === 'international') return tx.type === TransactionType.INTERNATIONAL;
        return true;
      });
    }
  ...
  useEffect(() => {
    if (activeFilter !== 'all' && filtered.length < 15 && hasMore && !loading && !loadingMore && !isFetching.current) {
      loadTransactions();
    }
  }, [filtered.length, activeFilter, hasMore, loading, loadingMore, loadTransactions]);
 ...
 useEffect(() => {
  ...
  const getTransactionIcon = (tx: Transaction) => {
    if (tx.status === 'PENDING') return <RotateCcw size={18} className="theme-text-secondary animate-spin-slow" />;
    
    if (tx.type === TransactionType.DEPOSIT) return <Plus size={18} className="text-green-600" />;
    if (tx.type === TransactionType.WITHDRAW) return <ArrowDown size={18} className="text-red-500" />;
    if (tx.type === TransactionType.INTERNATIONAL) return <Globe size={18} className="theme-primary-text" />;
    if (tx.type === TransactionType.RECHARGE) return <Smartphone size={18} className="theme-primary-text" />;
  ...
  const filters = [
    { id: 'all', label: t('history.filters.all'), icon: <LayoutList size={16} /> },
    { id: 'received', label: t('history.filters.received'), icon: <ArrowDownLeft size={16} /> },
    { id: 'sent', label: t('history.filters.sent'), icon: <ArrowUpRight size={16} /> },
    { id: 'deposits', label: t('history.filters.deposits'), icon: <Plus size={16} /> },
    { id: 'withdrawals', label: t('history.filters.withdrawals'), icon: <ArrowDown size={16} /> },
    { id: 'international', label: t('history.filters.intl'), icon: <Globe size={16} /> },
  ];
      ...
      >
        <div className="relative mb-3">
          ...
          <input
            ...
            className="w-full theme-bubble-bg rounded-full py-2 pl-10 pr-4 text-sm theme-text-main focus:bg-gray-200 transition-colors outline-none border border-transparent focus:border-[var(--primary-color)]"
          />
        </div>
                      ...
                      <motion.div 
                        ...
                        className={`flex items-start gap-4 cursor-pointer p-3 -mx-3 rounded-2xl transition-all duration-700 relative overflow-hidden group ${
                            highlightedTxId === tx.id
                                ? 'ring-2 ring-[var(--primary-color)] bg-[var(--primary-color)]/10 scale-[1.02] z-10'
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex justify-between items-baseline gap-2">
                            ...
                            <div className="shrink-0 text-right w-[100px]">
                               <span className={`font-black whitespace-nowrap block ${tx.role === 'payer' ? 'theme-text-main' : 'text-green-600'} ${tx.amount.toString().split('.')[0].length > 5 ? 'text-[10px]' : 'text-sm'}`}>
                                 {tx.role === 'payer' ? '-' : '+'} {tx.amount.toLocaleString(language === 'ht' ? 'ht-HT' : 'fr-HT')} {t('currency.symbol')}
                               </span>
                            </div>
                          </div>
                          <p className="theme-text-secondary text-xs truncate mt-0.5">{tx.description}</p>
                          ...
                          {tx.type === TransactionType.INTERNATIONAL && (
                            <div className="flex items-center gap-1 mt-1">
                              ...
                              <span className="text-[9px] font-bold text-blue-500 uppercase tracking-tighter">Verified Intl</span>
                            </div>
                          )}
                        </div>
                      </motion.div>
            ...
            <div ref={observerTarget} className="h-10 flex items-center justify-center">
              {loadingMore && (
                <div className="flex gap-1">
                  <div className="w-1.5 h-1.5 rounded-full theme-primary-bg animate-bounce" style={{ animationDelay: '0ms' }}></div>
                  <div className="w-1.5 h-1.5 rounded-full theme-primary-bg animate-bounce" style={{ animationDelay: '150ms' }}></div>
                  <div className="w-1.5 h-1.5 rounded-full theme-primary-bg animate-bounce" style={{ animationDelay: '300ms' }}></div>
                </div>
              )}
