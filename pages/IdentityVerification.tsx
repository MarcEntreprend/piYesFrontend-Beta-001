// pages\IdentityVerification.tsx
...
const IdentityVerification: React.FC<IdentityVerificationProps> = ({
    ...
    return (
      <div className="theme-card-bg min-h-screen flex flex-col p-8 animate-in fade-in duration-500">
        <header className="pt-8">
          <button onClick={() => navigate(-1)} className="theme-text-secondary">
            <ArrowLeft size={24} />
          </button>
        </header>
        <div className="flex-1 flex flex-col items-center justify-center text-center space-y-8">
          ...
          <div className="space-y-4">
            <h1 className="text-3xl font-bold theme-text-main">
              {t("identity_process.main_title")}
            </h1>
      ...
      <header className="p-6 flex items-center justify-between border-b border-white/10">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm font-bold tracking-tight">
            {t("identity_process.header_title")}
          </span>
        </div>
        <button
          onClick={() => setStep(0)}
      ...
      <PersonaWrapper>
        <div className="p-8 space-y-12 flex-1">
          <h2 className="text-3xl font-bold">
            {t("identity_process.step1_title")}
          </h2>
            ...
            <div className="flex gap-4">
              ...
              <div>
                <p className="font-bold text-sm">
                  {t("identity_process.step1_sub")}
                </p>
                <p className="text-xs text-white/50">
                  {t("identity_process.step1_hint")}
                </p>
              </div>
            </div>
          </div>
          <button
            onClick={nextStep}
            className="w-full bg-white text-black py-4 rounded-xl font-bold active:scale-95 transition-all"
          >
            {t("common.continue")}
          </button>
        </div>
  ...
  if (step === 2) {
    const docs = [
      {
        id: "id_card",
        label: t("identity_process.step2_id_card"),
        icon: <Landmark size={24} />,
      },
      {
        id: "passport",
        label: t("identity_process.step2_passport"),
        icon: <Globe size={24} />,
      },
    ];
    return (
      <PersonaWrapper>
        <div className="p-8 space-y-8 flex-1">
          <h2 className="text-2xl font-bold">
            {t("identity_process.step2_title")}
          </h2>
          <div className="space-y-3">
            {docs.map((doc) => (
              <button
                ...
                className="w-full flex items-center justify-between p-5 bg-white/5 border border-white/10 rounded-2xl hover:bg-white/10 transition-all text-left group active:scale-95"
              >
        ...
        <div className="flex-1 flex flex-col">
          <div className="p-8 text-center space-y-2">
            <h2 className="text-xl font-bold">
              {isVerso
                ? t("identity_process.verso_title")
                : t("identity_process.recto_title")}
            </h2>
          </div>
    ...
    return (
      <PersonaWrapper>
        <div className="p-8 flex-1 flex flex-col space-y-12">
          <h2 className="text-3xl font-bold">
            {t("identity_process.selfie_title")}
          </h2>
          <div className="flex-1 flex items-center justify-center">
            <div className="w-48 h-64 border-2 border-white/20 rounded-[100px] flex items-center justify-center bg-white/5">
              <UserCheck size={60} className="opacity-20" />
            </div>
          </div>
          <button
            onClick={nextStep}
            className="w-full bg-white text-black py-4 rounded-xl font-bold active:scale-95 transition-all"
          >
            {t("identity_process.selfie_ready")}
          </button>
        </div>
      </PersonaWrapper>
    );
    ...
    return (
      <PersonaWrapper>
        <div className="flex-1 flex flex-col items-center justify-center relative p-8">
          ...
          <button
            onClick={nextStep}
            className="mt-12 w-12 h-12 bg-white/10 rounded-full flex items-center justify-center border border-white/20 active:scale-90 transition-transform"
          >
            <Check size={20} />
          </button>
        </div>
  ...
  return (
    <div className="fixed inset-y-0 left-1/2 -translate-x-1/2 w-full max-w-md theme-card-bg flex flex-col p-8 items-center justify-center text-center space-y-12 animate-in zoom-in duration-500 z-130">
      <div className="w-24 h-24 bg-green-500 text-white rounded-4xl flex items-center justify-center shadow-2xl animate-in slide-in-from-top duration-700">
        <CheckCircle2 size={64} strokeWidth={1.5} />
      </div>
      <div className="space-y-4">
        <h1 className="text-3xl font-bold theme-text-main">
          {t("identity_process.congrats")}
        </h1>
        <p className="theme-text-secondary leading-relaxed max-w-xs">
          {t("identity_process.verified_msg")}
        </p>
      </div>
      <button
        onClick={() => {
