...
const MessagingHub: React.FC<MessagingHubProps> = ({ isTab = false }) => {
  ...
  return (
    <div
      className={`flex flex-col animate-in fade-in duration-500 ${isTab ? "" : "min-h-screen theme-card-bg pb-32"}`}
    >
      {!isTab && (
        <header className="px-6 pt-12 pb-4 theme-card-bg border-b theme-border flex items-center justify-between sticky top-0 z-40">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-2xl font-bold theme-text-main">
              {t("boutique.tabs.messages")}
            </h1>
          </div>
      ...
      <div className="p-6">
        <div className="relative mb-8">
          ...
          <input
            ...
            className="w-full theme-bubble-bg py-4 pl-12 pr-4 rounded-3xl outline-none theme-text-main text-sm border theme-border focus:theme-card-bg focus:shadow-md transition-all shadow-sm"
          />
        </div>
        ...
        {loading ? (
          <div className="space-y-4">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="h-24 theme-bubble-bg rounded-4xl shimmer"
              ></div>
            ))}
          </div>
        ) : (
          <div className="space-y-2">
            {filtered.map((chat) => (
              <div
                ...
                className="p-5 flex gap-5 hover:theme-bubble-bg transition-all cursor-pointer group rounded-4xl relative border theme-border shadow-sm active:scale-[0.98]"
              >
                ...
                <div className="flex-1 space-y-1 min-w-0">
                  <div className="flex justify-between items-center">
                    <div className="flex items-center gap-2 min-w-0">
                      <h4 className="font-black theme-text-main text-sm truncate">
                        {chat.counterparty.name}
                      </h4>
                      <span
                        className={`text-[8px] font-black px-1.5 py-0.5 rounded-full uppercase tracking-tighter shrink-0 ${chat.role === "seller" ? "bg-purple-100 text-purple-600" : "bg-blue-100 text-blue-600"}`}
                      >
                        {chat.role === "seller" ? "Ma Vente" : "Mon Achat"}
                      </span>
                    </div>
                    <span className="text-[10px] font-bold theme-text-secondary uppercase shrink-0">
                      {chat.lastTime}
                    </span>
                  </div>
                  <p className="text-[9px] font-black theme-primary-text uppercase tracking-[0.1em] truncate opacity-80">
                    {chat.adTitle}
                  </p>
                  <p
                    className={`text-xs truncate ${chat.unreadCount > 0 ? "theme-text-main font-black" : "theme-text-secondary opacity-60"}`}
                  >
                    {chat.lastMessage}
                  </p>
                </div>
