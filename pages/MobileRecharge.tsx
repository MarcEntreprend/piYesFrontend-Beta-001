...
const MobileRecharge: React.FC = () => {
  ...
  const executeRecharge = async (pin?: string) => {
    ...
    try {
      ...
      const errorMsg = e.response?.data?.error?.message || t('recharge.failure_msg');
      ...
      {selectedOperator && (
        <div className="theme-bubble-bg p-4 rounded-3xl border theme-border flex items-center gap-4 animate-in zoom-in duration-300">
          ...
          <div>
            ...
            <p className="font-bold theme-text-main text-lg">{selectedOperator.name}</p>
          </div>
        </div>
      )}
  ...
  const renderStepAmount = () => (
    ...
    <div className="space-y-3">
        <p className="text-[10px] font-black theme-text-secondary uppercase tracking-widest ml-1">
            {t('recharge.payment_method_subtitle') || 'Sélectionnez le mode de paiement'}
        </p>
        <div className="grid grid-cols-3 gap-3">
    {accounts.filter(acc => acc.balance > 0).map((acc) => {
      const currentAmount = amount || parseFloat(customAmount) || 0;
      const isInsufficient = currentAmount > 0 && acc.balance < currentAmount;
      ...
      return (
        ...
        >
          <BankIcon 
            ...
            color={acc.color} 
            size= {'xs'}
            className={selectedAccountId === acc.id ? 'border border-white/20' : ''}
          />
          <div className="text-center">
            <p className="text-[9px] font-black uppercase opacity-80 truncate w-full">{acc.label}</p>
            <p className={`text-[10px] font-bold ${isInsufficient ? 'text-red-500' : ''}`}>
      ...
      <div className="theme-bubble-bg p-8 rounded-[40px] border theme-border space-y-8 relative overflow-hidden">
        ...
        <div className="text-center space-y-2">
          ...
          <h2 className="text-4xl font-black theme-text-main">
            {(amount || parseFloat(customAmount)).toLocaleString()} <span className="text-xl opacity-50">{t('currency.symbol')}</span>
          </h2>
        </div>

        <div className="space-y-4 pt-4 border-t theme-border border-dashed">
          <div className="flex justify-between items-center">
            <span className="text-xs font-bold theme-text-secondary">{t('recharge.phone_placeholder')}</span>
            <span className="font-black theme-text-main">{phoneNumber}</span>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-xs font-bold theme-text-secondary">{t('recharge.operator_detected')}</span>
            <div className="flex items-center gap-2">
              <div className="w-4 h-4 rounded-full" style={{ backgroundColor: selectedOperator?.color }}></div>
              <span className="font-black theme-text-main">{selectedOperator?.name}</span>
            </div>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-xs font-bold theme-text-secondary">{t('interbank.source_label')}</span>
            <span className="font-black theme-text-main">{accounts.find(a => a.id === selectedAccountId)?.label}</span>
          </div>
        </div>
      </div>
  ...
  const renderStepReceipt = () => (
    <div className="space-y-8 animate-in zoom-in duration-500">
      <div className="flex flex-col items-center text-center space-y-4">
        <div className="w-20 h-20 bg-green-500 text-white rounded-[32px] flex items-center justify-center shadow-2xl shadow-green-500/20 animate-bounce">
          <CheckCircle2 size={40} />
        </div>
        <div className="space-y-1">
          <h2 className="text-2xl font-black theme-text-main">{t('recharge.success_msg')}</h2>
          <p className="text-sm theme-text-secondary">{t('common.generated_on', { date: new Date().toLocaleDateString(), at: '', time: new Date().toLocaleTimeString() })}</p>
        </div>
      </div>

      <div className="theme-bubble-bg p-6 rounded-[32px] border theme-border space-y-4">
        ...
        <div className="flex justify-between items-center">
          <span className="text-xs font-bold theme-text-secondary">{t('receipt.amount_label')}</span>
          <span className="text-xl font-black theme-text-main">{(amount || parseFloat(customAmount)).toLocaleString()} {t('currency.symbol')}</span>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-3">
        <Button
            variant="utility"
            fullWidth
            onClick={() => navigate(`/receipt/${receiptId}?type=recharge&role=payer`)}
  ...
  return (
    <div className="min-h-screen theme-bg flex flex-col pb-32">
      <PageHeader
        ...
        onBack={() => step === 'number' ? navigate(-1) : setStep(step === 'amount' ? 'number' : step === 'confirm' ? 'amount' : 'number')}
      />
        ...
        <div className="p-6 space-y-4 max-h-[60vh] overflow-y-auto no-scrollbar">
          {contacts.filter(c => c.phone).length > 0 ? (
            contacts.filter(c => c.phone).map(contact => (
              ...
              >
                ...
                <div className="text-left">
                  ...
                  <p className="text-xs theme-text-secondary">{contact.phone}</p>
                </div>
              </button>
            ))
          ) : (
            <div className="text-center py-10 space-y-4">
              ...
              <p className="text-sm theme-text-secondary">{t('history.empty')}</p>
            </div>
          )}
        </div>
      </Modal>
