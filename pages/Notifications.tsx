...
const Notifications: React.FC = () => {
  ...
  const { notifications, loading, markAllRead, markRead, unreadCount, refresh } = useNotifications();
  ...
  const handleClearOne = (id: string, e: React.MouseEvent) => {
    ...
    localStorage.setItem('piyes_cleared_notifications', JSON.stringify(newCleared));
  };
  ...
  const getIcon = (type: string) => {
    switch (type) {
        case 'transfer_in':
        case 'transfer_received': return <ArrowDownLeft size={20} className="text-green-500" />;
        case 'transfer_out': return <ArrowUpRight size={20} className="theme-primary-text" />;
        ...
        case 'request': return <QrCode size={20} className="theme-primary-text" />;
        case 'scheduled_request':
        case 'scheduled_created':
        case 'scheduled_confirmed':
        case 'scheduled_cancelled': return <CalendarClock size={20} className="theme-primary-text" />;
        default: return <Info size={20} className="theme-text-secondary" />;
    }
  };
  ...
  const handleNotifClick = (notif: Notification) => {
    ...
    if (notif.type === 'scheduled_request') {
      ...
      try {
        ...
        if (extra?.receiverUserId) {
          ...
          sessionStorage.setItem('pending_contact_from_scheduler', JSON.stringify({
    ...
    if (notif.type === 'scheduled_confirmed') {
      ...
      navigate(`/scheduler?tab=incoming${schedId ? `&highlight=${schedId}` : ''}`);
      return;
    }
    ...
    if (notif.type === 'scheduled_cancelled') {
      const schedId = notif.data?.targetId || notif.targetId;
      navigate(`/scheduler?tab=outgoing${schedId ? `&highlight=${schedId}` : ''}`);
      return;
    }
    ...
    if (notif.data?.params) {
        Object.entries(notif.data.params).forEach(([key, val]) => params.append(key, val as string));
    }
    if (targetId) {
    ...
    const finalUrl = params.toString() ? `${route}?${params.toString()}` : route;
    navigate(finalUrl);
};
  ...
  const formatTime = (isoString: string) => {
    ...
    if (diff < 86400000) return `${t('common.today')} • ${date.getHours()}:${date.getMinutes().toString().padStart(2, '0')}`;
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar px-6">
        {loading && notifications.length === 0 ? (
            <div className="py-6 space-y-6">
                {[1,2,3,4].map(i => <div key={i} className="h-20 theme-bubble-bg rounded-2xl shimmer"></div>)}
            </div>
        ) : (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0 space-y-0.5">
                          <div className="flex justify-between items-start gap-2">
                              <h3 className={`text-sm leading-tight theme-text-main truncate pr-6 ${!notif.isRead ? 'font-black' : 'font-bold'}`}>
                                {t(`notifications.types.${notif.type}.title`, { ...notif.data, name: notif.data?.name || notif.title, amount: notif.amount || notif.data?.amount })}
                              </h3>
                              <button 
                          ...
                          <p className="theme-text-secondary text-xs leading-relaxed line-clamp-2">
                            {t(`notifications.types.${notif.type}.body`, { ...notif.data, name: notif.data?.name || notif.body, amount: notif.amount || notif.data?.amount })}
                          </p>
                          <div className="flex items-center justify-between mt-1">
                            {notif.amount ? (
                                <p className="font-black theme-text-main text-xs">{notif.amount}</p>
                            ) : <div />}
                            <p className="text-[9px] font-bold theme-text-secondary opacity-60 uppercase tracking-tighter">{formatTime(notif.timestamp)}</p>
                          </div>
                      </div>
                    </motion.div>
                ))}
        ...
        {!loading && visibleNotifications.length === 0 && (
            <div className="p-12 text-center space-y-4">
                ...
                <p className="theme-text-secondary text-sm">{t('notifications.empty')}</p>
            </div>
        )}
      </div>
    </div>
