...
import { 
  ArrowLeft, Check, ChevronRight, Shield, Rocket, 
  Gem, HelpCircle, Star, Sparkles, Info
} from 'lucide-react';
...
const Plans: React.FC = () => {
  ...
  const handleSelectPlan = (plan: Plan) => {
    ...
    triggerSensitiveAction(() => {
        ...
        showToast(t('plans.toast_subscribe_sent', { name: plan.name }), 'success');
        navigate('/');
    });
  };
      ...
      <main className="flex-1 p-6 space-y-10 overflow-y-auto no-scrollbar">
        <div className="space-y-2">
            <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[10px] font-black theme-primary-text uppercase tracking-widest">{t('plans.exclusive_offers')}</span>
            </div>
            <h2 className="text-2xl font-black theme-text-main leading-tight">{t('plans.subtitle')}</h2>
        </div>
                ...
                return (
                    <div 
                        ...
                        className={`p-6 rounded-[40px] border-2 transition-all duration-300 relative overflow-hidden group cursor-pointer ${
                            ...
                            ? 'theme-bubble-bg border-[var(--primary-color)] shadow-xl' 
                    ...
                    >
                        ...
                        <div className="flex gap-5 items-start mb-6">
                            ...
                            <div className="space-y-1">
                                <h3 className="text-xl font-black theme-text-main tracking-tight">{plan.name}</h3>
                                <div className="flex items-baseline gap-1">
                                    <span className="text-2xl font-black theme-primary-text">{plan.price.toLocaleString('fr-HT')} G.</span>
                                    <span className="text-[10px] font-bold theme-text-secondary">{t('plans.month')}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="p-4 bg-gray-50/50 dark:bg-white/5 rounded-3xl border theme-border mb-6">
                            <p className="text-[10px] font-black theme-text-secondary uppercase tracking-[0.1em] italic">
                                {t('plans.estimated_value', { amount: plan.estimatedValue.toLocaleString('fr-HT') })}
                            </p>
                        </div>
                        ...
                        <ul className="space-y-3 mb-8">
                            {perksArray.map((perk: string, i: number) => (
                                <li key={i} className="flex items-start gap-3">
                                    ...
                                    <span className="text-xs font-bold theme-text-secondary leading-tight">{perk}</span>
                                </li>
                            ))}
                        </ul>
                        ...
                        <div className="space-y-2">
                            ...
                            >
                                {plan.id === 'basic' ? t('plans.active_badge') : t('plans.choose')}
        ...
        <div className="p-6 theme-bubble-bg rounded-[32px] border theme-border flex gap-4 items-start shadow-inner">
        ...
        <div className="flex flex-col items-center gap-2 opacity-30 py-8">
            ...
            <p className="text-[9px] font-black theme-text-main uppercase tracking-[0.2em]">{t('plans.secure_payment')}</p>
        </div>
      </main>
    </div>
  );
