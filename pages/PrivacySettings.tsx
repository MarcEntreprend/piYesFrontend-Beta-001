...
const PrivacySettings: React.FC = () => {
  ...
  const handleBlockOption = (
    ...
    const field =
      type === "requests" ? "blockRequestsFrom" : "blockTransfersFrom";
  ...
  const confirmBlock = (tag: string, type: "requests" | "transfers") => {
    setConfirmModal({
      ...
      message: t("privacy_settings.confirm_block_msg", {
        type: type === "requests" ? "demandes" : "transferts",
        tag,
      }),
      onConfirm: () => {
        if (!settings) return;
  ...
  const Section = ({
    ...
    <div className="mb-8">
      <h3 className="px-6 text-[11px] font-bold theme-text-secondary uppercase tracking-[0.15em] mb-4">
        {title}
      </h3>
  ...
  const Option = ({
    ...
    <div className="flex items-center gap-2">
      <button
        onClick={onClick}
        className={`flex-1 flex items-center justify-between p-4 rounded-2xl transition-all active:scale-[0.98] ${active ? "theme-bubble-bg border border-(--primary-color)/20" : "hover:theme-bubble-bg"}`}
      >
        <span
          className={`text-sm font-bold ${active ? "theme-primary-text" : "theme-text-main"}`}
        >
          {label}
        </span>
        <div
          className={`w-5 h-5 rounded-md border-2 flex items-center justify-center transition-all ${active ? "theme-primary-bg border-transparent" : "theme-border"}`}
        >
  ...
  const Toggle = ({
    ...
    <div className="flex items-center gap-2">
      ...
      >
        ...
        <div
          className={`w-12 h-6 rounded-full p-1 transition-all duration-300 ${active ? "theme-primary-bg" : "theme-bubble-bg border theme-border"}`}
        >
          <div
            className={`w-4 h-4 bg-white rounded-full transition-all duration-300 transform ${active ? "translate-x-6" : "translate-x-0"}`}
          ></div>
        </div>
      </button>
      {help && (
    ...
    <div className="theme-card-bg min-h-screen pb-32">
      ...
      <div className="animate-in fade-in duration-500 py-6">
        <Section title={t("privacy_settings.requests_section")}>
          <Option
            label={
              settings.blockRequestsFrom === "none"
                ? t("privacy_settings.allow_everyone")
                : t("privacy_settings.block_everyone")
            }
            active={settings.blockRequestsFrom === "everyone"}
            onClick={() =>
              handleBlockOption(
                "requests",
                settings.blockRequestsFrom === "everyone" ? "none" : "everyone",
              )
            }
        ...
        <Section title={t("privacy_settings.transfers_section")}>
          <Option
            label={
              settings.blockTransfersFrom === "none"
                ? t("privacy_settings.allow_transfers_everyone")
                : t("privacy_settings.block_transfers_everyone")
            }
            active={settings.blockTransfersFrom === "everyone"}
            onClick={() =>
              handleBlockOption(
                "transfers",
                settings.blockTransfersFrom === "everyone"
                  ? "none"
                  : "everyone",
              )
            }
        ...
        <Section title={t("privacy_settings.others_section")}>
          <Toggle
            ...
            onClick={() =>
              updateSetting({
                allowAnonymousTransfers: !settings.allowAnonymousTransfers,
              })
            }
          ...
          <Toggle
            ...
            onClick={() =>
              updateSetting({ hideTagInReceipts: !settings.hideTagInReceipts })
            }
          ...
          <Toggle
            ...
            onClick={() =>
              updateSetting({
                requestsOnlyFromFriends: !settings.requestsOnlyFromFriends,
              })
            }
        ...
        {searchModal && (
          <div className="p-8 space-y-6 h-[80vh] flex flex-col">
            <div className="flex justify-between items-center">
              <h3 className="text-xl font-bold theme-text-main">
                {searchModal.type === "requests"
                  ? t("privacy_settings.block_requests_title")
                  : t("privacy_settings.block_transfers_title")}
              </h3>
              <button
            ...
            <div className="relative">
              <Search
                className="absolute left-4 top-1/2 -translate-y-1/2 theme-text-secondary opacity-40"
                size={18}
              />
              <input
                type="text"
                ...
                className="w-full theme-bubble-bg p-4 pl-12 rounded-2xl outline-none theme-text-main border theme-border focus:border-(--primary-color)"
              />
            </div>
              ...
              {contacts
                ...
                .map((contact) => (
                  <button
                    ...
                    className="w-full flex items-center justify-between p-4 theme-bubble-bg rounded-2xl border theme-border hover:border-(--primary-color) transition-all"
                  >
              ...
              {searchTerm && !contacts.some((c) => c.tag === searchTerm) && (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-left">
                      <p className="text-sm font-bold theme-text-main">
                        {t("privacy_settings.block_tag_exact", {
                          tag: searchTerm,
                        })}
                      </p>
                      <p className="text-[10px] theme-text-secondary">
                        {t("privacy_settings.block_tag_hint")}
                      </p>
                    </div>
                  </div>
                  <UserX size={18} className="text-red-500" />
