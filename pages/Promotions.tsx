...
import { ArrowLeft, Gift, Users, Star, ChevronRight, Sparkles, Zap, Ticket, Share2 } from 'lucide-react';
    ...
    />
       ...
       <div className="mt-12 px-6">
            <button 
                ...
                className="w-full p-6 bg-gradient-to-br from-gray-50 to-white dark:from-white/5 dark:to-transparent rounded-[32px] border theme-border flex gap-5 items-center text-left group active:scale-[0.98] transition-all"
            >
                ...
                <div className="flex-1 space-y-1">
                    <p className="text-xs font-bold theme-text-main leading-tight">{t('promos.club.title')}</p>
      ...
      <div className="animate-in fade-in duration-500">
        <div className="p-6">
            <div className="theme-primary-bg rounded-[32px] p-8 text-white relative overflow-hidden shadow-xl">
                ...
                <div className="relative z-10 space-y-4">
                    ...
                    <div className="space-y-1">
                        <h2 className="text-2xl font-black leading-tight">{t('promos.banner.title')}</h2>
                        <p className="text-xs opacity-80 max-w-[200px]">{t('promos.banner.desc')}</p>
                    </div>
        ...
        <div className="px-6 space-y-8">
            <section className="space-y-4">
                <h3 className="text-[11px] font-bold theme-text-secondary uppercase tracking-[0.2em]">{t('promos.available')}</h3>
                    ...
                    {activePromos.map(promo => (
                        ...
                        >
                            <div className="flex items-center gap-5">
                                <div className={`w-14 h-14 ${promo.color} rounded-2xl flex items-center justify-center shrink-0 border theme-border group-hover:scale-105 transition-transform`}>
                                    {promo.icon}
                                </div>
                                <div className="space-y-1">
                                    <div className="flex items-center gap-2">
                                        <h4 className="font-bold theme-text-main text-sm">{promo.title}</h4>
                                        <span className="text-[8px] font-bold bg-gray-100 theme-text-secondary px-2 py-0.5 rounded-full uppercase tracking-tighter">{promo.badge}</span>
                                    </div>
                                    <p className="text-[10px] theme-text-secondary leading-relaxed line-clamp-2 max-w-[180px]">
                                        {promo.description}
                                    </p>
                                </div>
                            </div>
                            <ChevronRight size={18} className="theme-text-secondary opacity-30 group-hover:opacity-100 transition-opacity" />
                        </div>
                    ))}
                </div>
            </section>
            ...
            <section className="space-y-4">
                <h3 className="text-[11px] font-bold theme-text-secondary uppercase tracking-[0.2em]">{t('promos.loyalty.title')}</h3>
                <div className="p-6 theme-bubble-bg border theme-border rounded-[32px] space-y-6">
                    <div className="flex justify-between items-center">
                        <div className="flex items-center gap-3">
                            <Star className="text-amber-500 fill-amber-500" size={20} />
                            <span className="font-bold theme-text-main text-sm">{t('promos.loyalty.points')}</span>
                        </div>
                        <span className="text-xl font-black theme-primary-text">1,250</span>
                    </div>
