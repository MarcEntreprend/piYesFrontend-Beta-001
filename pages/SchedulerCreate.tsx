...
import {
  ArrowLeft, CalendarClock, Info, AlertCircle,
  ChevronRight, Loader2, UserPlus, Check, XCircle
} from 'lucide-react';
...
const SchedulerCreate: React.FC = () => {
  ...
  const [payerUserId, setPayerUserId] = useState(searchParams.get('payerUserId') || '');
  ...
  // Result
  const [created, setCreated] = useState<{ qrToken: string; qrExpiresAt: string; id: string } | null>(null);
  ...
  useEffect(() => {
    if (!payerUserId) { setIsMutualFriend(false); return; }
    setCheckingFriendship(true);

    api.getFriendshipStatus(payerUserId)
      .then((friendship: any) => {
 ...
 const handleSelectContact = (contact: Partial<Contact>) => {
  ...
  const activeCount = reminders.reduce((a, r) => a + (r.time1Active ? 1 : 0) + (r.time2Active ? 1 : 0), 0);
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setQrUrl(url);
      setCreated({ qrToken: result.qrToken!, qrExpiresAt: result.qrExpiresAt!, id: result.id });
    } catch (e: any) {
  ...
  const canCreate = payerName.trim().length > 0 && parseFloat(amount) > 0 && dueDate && activeCount > 0;
    ...
    return (
      <div className="theme-card-bg min-h-screen flex flex-col">
        <header className="px-6 pt-12 pb-6 border-b theme-border flex items-center gap-4 sticky top-0 theme-card-bg z-10">
          <button onClick={() => navigate('/scheduler')} className="p-2 -ml-2 theme-text-secondary active:scale-90">
            <ArrowLeft size={24} />
          </button>
          <h1 className="text-lg font-bold theme-text-main">{t('scheduler.create.success_title')}</h1>
        </header>
        ...
        <div className="flex-1 p-6 flex flex-col items-center gap-8 animate-in fade-in">
          ...
          <div className="text-center space-y-2">
            <h2 className="text-2xl font-black theme-text-main">{t('scheduler.create.waiting_confirmation')}</h2>
            <p className="text-sm theme-text-secondary">{t('scheduler.create.share_instruction', { name: payerName })}</p>
          </div>

          <div className="bg-white p-4 rounded-[32px] shadow-xl border theme-border">
  ...
  return (
    <div className="theme-card-bg min-h-screen flex flex-col pb-32">
      ...
      <p className="text-[10px] theme-text-secondary">{t('scheduler.create.subtitle')}</p>

        {/* Titre (optionnel) */}
        <div className="space-y-2">
          <label className="text-[10px] font-black theme-text-secondary uppercase tracking-widest px-1">
            {t('scheduler.create.title_label')} <span className="opacity-50">{t('scheduler.create.optional')}</span>
          </label>
          <input
            type="text"
            value={title}
            onChange={e => setTitle(e.target.value)}
            placeholder={t('scheduler.create.title_placeholder', { name: payerName || '...' })}
            className="w-full theme-bubble-bg p-4 rounded-2xl outline-none theme-text-main border theme-border focus:border-[var(--primary-color)] transition-all font-bold"
          />
        </div>
        ...
        <div className="space-y-2">
          <label className="text-[10px] font-black theme-text-secondary uppercase tracking-widest px-1">
            {t('scheduler.create.payer_label')} <span className="text-red-400">{t('scheduler.create.required')}</span>
          </label>
            ...
            <div className="flex items-center gap-3 theme-primary-bg text-white rounded-full px-4 py-3 w-fit max-w-full">
              <div className="w-8 h-8 bg-white/20 rounded-full flex items-center justify-center shrink-0">
                <span className="text-xs font-black">
                  {payerName.split(' ').map((w: string) => w[0]).join('').substring(0, 2).toUpperCase()}
                </span>
              </div>
              <span className="font-bold text-sm truncate">{payerName}</span>
              <button
                type="button"
                onClick={() => { setPayerName(''); setPayerUserId(''); setSearchQuery(''); setIsMutualFriend(false); }}
          ...
          {payerUserId && payerName && !searchQuery && (
            <div className={`flex items-center gap-2 px-2 text-[10px] font-bold ${isMutualFriend ? 'text-green-500' : 'text-amber-500'}`}>
              {checkingFriendship ? (
                <Loader2 size={12} className="animate-spin" />
              ) : isMutualFriend ? (
                <><Check size={12} /> {t('scheduler.create.mutual_friend')}</>
              ) : (
                <><AlertCircle size={12} /> {t('scheduler.create.not_mutual_friend')}</>
              )}
            </div>
          )}
        </div>
        ...
        <div className="space-y-2">
          <label className="text-[10px] font-black theme-text-secondary uppercase tracking-widest px-1">
            {t('scheduler.create.amount_label')} <span className="text-red-400">{t('scheduler.create.required')}</span>
          </label>
        ...
        <div className="space-y-2">
          <label className="text-[10px] font-black theme-text-secondary uppercase tracking-widest px-1">
            {t('scheduler.create.due_date_label')} <span className="text-red-400">{t('scheduler.create.required')}</span>
          </label>
          <input
            type="date"
            value={dueDate}
            ...
            className="w-full theme-bubble-bg p-4 rounded-2xl outline-none theme-text-main border theme-border focus:border-[var(--primary-color)] transition-all font-bold text-sm"
          />
        </div>
        ...
        {reminders.length > 0 && (
          <div className="space-y-3">
            <div className="flex items-center justify-between px-1">
              ...
              <span className="text-[9px] theme-primary-text font-bold">{activeCount} {activeCount > 1 ? t('scheduler.create.actives') : t('scheduler.create.active')}</span>
            </div>
                      ...
                      <td key={r.date} className="py-1.5 px-0.5">
                        <button
                          ...
                          className={`w-10 h-8 rounded-xl text-[9px] font-bold transition-all active:scale-90 ${
                            r.time1Active ? 'theme-primary-bg text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-300 dark:text-gray-600'
                          }`}
                        >
                    ...
                    {reminders.map(r => (
                      <td key={r.date} className="py-1.5 px-0.5">
                        <button
                          ...
                          className={`w-10 h-8 rounded-xl text-[9px] font-bold transition-all active:scale-90 ${
                            r.time2Active ? 'theme-primary-bg text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-300 dark:text-gray-600'
                          }`}
                        >
        ...
        <button
          ...
          className="w-full py-5 theme-primary-bg text-white rounded-[24px] font-black shadow-xl active:scale-95 transition-all uppercase tracking-widest disabled:opacity-40 flex items-center justify-center gap-3"
        >
          {loading ? <Loader2 size={20} className="animate-spin" /> : <CalendarClock size={20} />}
          {t('scheduler.create.btn_create')}
        </button>
      </div>

      {/* Modal demande d'ami nécessaire */}
      <Modal isOpen={showFriendRequestModal} onClose={() => setShowFriendRequestModal(false)} type="centered">
        <div className="p-8 space-y-6 text-center">
          <div className="w-16 h-16 bg-amber-50 text-amber-500 rounded-full flex items-center justify-center mx-auto">
            <UserPlus size={32} />
          </div>
          <div className="space-y-2">
            <h3 className="text-xl font-black theme-text-main">{t('scheduler.create.friend_request_modal.title')}</h3>
            <p className="text-sm theme-text-secondary">
              {t('scheduler.create.friend_request_modal.desc', { name: payerName })}
            </p>
          </div>
          <div className="flex flex-col gap-3">
            ...
            >
              {sendingFriendRequest ? <Loader2 size={18} className="animate-spin" /> : <UserPlus size={18} />}
              {friendRequestSent ? t('scheduler.create.friend_request_modal.btn_sent') : t('scheduler.create.friend_request_modal.btn_send')}
            </button>
            <button onClick={() => setShowFriendRequestModal(false)} className="w-full py-3 theme-text-secondary font-bold text-sm">
