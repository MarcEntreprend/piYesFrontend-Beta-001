...
const Settings: React.FC<SettingsProps> = ({ user, currentTheme, currentFontSize, onThemeChange, onFontSizeChange, onLogout }) => {
  ...
  const themes = [
    ...
    { id: 'bleu_cendre', label: t('settings.themes.bleu_cendre'), color: '#24A1DE' },
  ];
  ...
  useEffect(() => {
    ...
    if (scrollTo) {
        setTimeout(() => {
            ...
            if (el) {
                ...
                el.classList.add('ring-4', 'ring-[var(--primary-color)]', 'ring-opacity-20', 'rounded-2xl', 'transition-all', 'duration-500');
    ...
    return () => window.removeEventListener('piyes:reset_settings', handleReset);
  }, [searchParams]);

  const SettingItem = ({ icon, label, sublabel, onClick, rightElement, id }: any) => (
    <button 
      id={id}
      onClick={onClick}
    ...
    >
      <div className="flex items-center gap-4">
        ...
        <div className="text-left">
          ...
          {sublabel && <p className="text-[10px] theme-text-secondary">{sublabel}</p>}
        </div>
      </div>
  ...
  return (
    <div className="theme-card-bg min-h-screen pb-32">
      ...
      >
        <div className="flex items-center gap-4">
          <div className="w-16 h-16 theme-bubble-bg rounded-full flex items-center justify-center font-black theme-primary-text text-2xl border-2 border-[var(--primary-color)] shadow-sm group-active:scale-95 transition-transform overflow-hidden">
            {user.avatarUrl ? <img src={user.avatarUrl} alt="" className="w-full h-full object-cover" /> : user.name.split(' ').map(n => n[0]).join('')}
          </div>
          <div className="space-y-0.5">
            <h2 className="text-lg font-bold theme-text-main group-hover:theme-primary-text transition-colors">{user.name}</h2>
            <p className="text-xs font-bold theme-primary-text">{user.tag || '@piyes.user'}</p>
      ...
      <div className="animate-in fade-in duration-500">
        ...
        <div className="px-2">
          <SettingItem 
            ...
            rightElement={<span className="text-xs font-bold theme-primary-text">{languages.find(l => l.id === language)?.label}</span>}
            onClick={() => setShowLanguageModal(true)}
          />
          <div className="w-full" id="set-theme">
            <button 
              onClick={() => { setShowThemes(!showThemes); setShowFontSizes(false); }}
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div className="text-left">
                  <p className="font-bold theme-text-main text-sm">{t('settings.theme')}</p>
                  <p className="text-[10px] theme-text-secondary capitalize">
                    {themes.find(th => th.id === currentTheme)?.label || currentTheme}
                  </p>
                </div>
              </div>
              <ChevronRight size={16} className={`theme-text-secondary transition-transform duration-300 ${showThemes ? 'rotate-90' : 'opacity-30'}`} />
            </button>
                ...
                {themes.map(t_theme => (
                  <button
                    ...
                    className={`w-full flex items-center justify-between p-3 rounded-xl border-2 transition-all ${
                      ...
                      ? 'border-[var(--primary-color)] bg-[var(--bubble-bg)]' 
                      : 'theme-border bg-transparent opacity-60'
                    }`}
                  >
                    <div className="flex items-center gap-3">
                      <div className="w-4 h-4 rounded-full border border-white/20" style={{ backgroundColor: t_theme.color }}></div>
                      <span className="text-xs font-bold theme-text-main">{t_theme.label}</span>
                    </div>
                    {currentTheme === t_theme.id && <Check size={14} className="theme-primary-text" />}
                  </button>
                ))}
              </div>
            )}
          ...
          <div className="w-full" id="set-font-size">
            <button 
              onClick={() => { setShowFontSizes(!showFontSizes); setShowThemes(false); }}
              className="w-full flex items-center justify-between p-4 hover:theme-bubble-bg transition-all group rounded-2xl"
            >
              <div className="flex items-center gap-4">
                <div className="w-10 h-10 theme-bubble-bg rounded-xl flex items-center justify-center theme-primary-text">
                ...
                <div className="text-left">
                  <p className="font-bold theme-text-main text-sm">{t('settings.font_size')}</p>
                  <p className="text-[10px] theme-text-secondary capitalize">
                    {fontSizes.find(s => s.id === currentFontSize)?.label || currentFontSize}
                  </p>
                </div>
              </div>
              <ChevronRight size={16} className={`theme-text-secondary transition-transform duration-300 ${showFontSizes ? 'rotate-90' : 'opacity-30'}`} />
            </button>
            ...
            {showFontSizes && (
              <div className="px-4 pb-4 space-y-2 animate-in slide-in-from-top duration-300 theme-card-bg rounded-2xl">
                {fontSizes.map(size => (
                  ...
                  >
                    <span className="text-xs font-bold theme-text-main">{size.label}</span>
                    {currentFontSize === size.id && <Check size={14} className="theme-primary-text" />}
                  </button>
                ))}
              </div>
            )}
          ...
          <SettingItem 
            ...
            icon={<Loader2 size={20} className={loading ? 'animate-spin' : ''} />} 
        ...
        <div className="p-8 space-y-8">
          <div className="flex justify-between items-center">
            <div className="space-y-1">
              <h2 className="text-2xl font-black theme-text-main">{t('settings.language_modal_title')}</h2>
              <p className="text-[10px] theme-text-secondary font-bold uppercase tracking-widest">{t('settings.language_modal_sub')}</p>
            </div>
            <button onClick={() => setShowLanguageModal(false)} className="p-2 theme-bubble-bg rounded-full theme-text-secondary"><X size={20}/></button>
          </div>
          ...
          <div className="space-y-3 pb-6">
            {languages.map(lang => (
              <button 
                key={lang.id}
                onClick={() => { setLanguage(lang.id as Language); setShowLanguageModal(false); }}
              ...
              >
                <div className="flex items-center gap-5">
                  <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full ${language === lang.id ? 'bg-white/20' : 'bg-white shadow-sm'}`}>
                    ...
                    <span className={`text-[11px] font-black tracking-tight ${language === lang.id ? 'text-white' : 'text-gray-900'}`}>{lang.code}</span>
                  </div>
      ...
      <Modal isOpen={showLogoutAllConfirm} onClose={() => setShowLogoutAllConfirm(false)}>
        <div className="p-8 space-y-6">
          <div className="w-16 h-16 bg-red-50 dark:bg-red-900/10 rounded-full flex items-center justify-center text-red-500 mx-auto">
            <Shield size={32} />
          </div>
          <div className="text-center space-y-2">
            <h2 className="text-2xl font-black theme-text-main">{t('settings.logout_all_confirm_title')}</h2>
            <p className="text-sm theme-text-secondary">{t('settings.logout_all_confirm_desc')}</p>
          </div>
          <div className="flex gap-3">
            <Button
              variant="secondary"
