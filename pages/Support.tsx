...
import { ArrowLeft, MessageSquare, Phone, Mail, Clock, Headphones } from 'lucide-react';
    ...
    <div className="theme-card-bg min-h-screen flex flex-col animate-in slide-in-from-bottom duration-500">
      ...
      <div className="p-6 space-y-10 flex-1 flex flex-col items-center justify-center text-center">
        ...
        <div className="space-y-2">
            <h2 className="text-2xl font-bold theme-text-main">{t('support.need_help')}</h2>
            <p className="theme-text-secondary text-sm max-w-xs mx-auto">{t('support.sub')}</p>
        </div>
        ...
        <div className="w-full space-y-3">
            <button className="w-full flex items-center gap-4 p-5 theme-bubble-bg rounded-2xl border theme-border hover:bg-purple-100 dark:hover:bg-purple-900/20 transition-all active:scale-95">
                ...
                <div className="text-left">
                    <p className="font-bold theme-text-main text-sm">{t('support.chat.label')}</p>
                    <p className="text-[10px] theme-text-secondary">{t('support.chat.sub')}</p>
                </div>
            </button>
            ...
            <button className="w-full flex items-center gap-4 p-5 theme-bubble-bg rounded-2xl border theme-border hover:bg-purple-100 dark:hover:bg-purple-900/20 transition-all active:scale-95">
                ...
                <div className="text-left">
                    <p className="font-bold theme-text-main text-sm">{t('support.call.label')}</p>
                    <p className="text-[10px] theme-text-secondary">+509 22XX-XXXX {t('support.call.sub')}</p>
                </div>
            </button>
            ...
            <button className="w-full flex items-center gap-4 p-5 theme-bubble-bg rounded-2xl border theme-border hover:bg-purple-100 dark:hover:bg-purple-900/20 transition-all active:scale-95">
                ...
                <div className="text-left">
                    <p className="font-bold theme-text-main text-sm">{t('support.email.label')}</p>
                    <p className="text-[10px] theme-text-secondary">{t('support.email.sub')}</p>
                </div>
            </button>
        </div>
