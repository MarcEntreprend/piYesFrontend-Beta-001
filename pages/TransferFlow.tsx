...
const TransferFlow: React.FC<TransferFlowProps> = ({ user, onUpdateUser }) => {
  ...
  const getPriorityKey = (contact: Contact | Partial<Contact>) => {
    ...
    if (contact.phone)
      return contact.phone.startsWith("+")
        ? contact.phone
        : `+${contact.phone}`;
  ...
  useEffect(() => {
    ...
    if (recipientParam) {
      ...
      const qNoPrefix =
        q.startsWith("@") || q.startsWith("+") ? q.substring(1) : q;
    ...
    api
      ...
      .catch((e: any) => {
        const msg =
          ...
          e?.message ||
          t("transfer.recipient_not_found_error");
  ...
  const handleTransfer = async (pin?: string) => {
    ...
    try {
      ...
      if (tx.recipientId) {
        ...
        else if (/^\d{8}$/.test(searchVal) || searchVal.startsWith("+509"))
          contactData.phone = searchVal;
        try {
  ...
  const getRecipientInitials = () => {
    ...
    if (
      keyValue.startsWith("@") &&
      getRecipientType(keyValue) === RecipientType.TAG
    )
      return keyValue.substring(1, 3).toUpperCase();
  ...
  const handleBack = () => {
    if (step === 1) {
      ...
      navigate(
        `/keys?modal=pix&link=${encodeURIComponent(originalLink || "")}`,
      );
    } else {
      setStep(step - 1);
    }
      ...
      <div className="flex-1 px-0 flex flex-col overflow-y-auto no-scrollbar pb-6 pt-6">
        {step === 1 ? (
          <div className="flex-1 animate-in slide-in-from-right duration-300">
            <h2 className="text-2xl font-bold mb-8 theme-text-main px-6">
              {t("transfer.step_who")}
            </h2>
              ...
              <div className="space-y-6 mt-4">
                {favorites.length > 0 && (
                  <div className="space-y-3">
                    <h3 className="text-[10px] font-black theme-text-secondary uppercase tracking-widest px-8">
                      {t("transfer.favorites")}
                    </h3>
                ...
                <div className="space-y-3">
                  <h3 className="text-[10px] font-black theme-text-secondary uppercase tracking-widest px-8">
                    {t("transfer.all_contacts")}
                  </h3>
                  <div className="bg-white theme-bubble-bg rounded-4xl overflow-hidden border theme-border mx-6">
                    {contacts
          ...
          <div className="flex-1 animate-in slide-in-from-right duration-300 px-6">
            ...
            <div className="flex items-center border-b-2 theme-border pb-2 focus-within:border-(--primary-color) transition-colors mb-10">
          ...
          <div className="flex-1 animate-in slide-in-from-right duration-300 px-6">
            <h2 className="text-2xl font-bold mb-8 theme-text-main">
              {t("transfer.step_review")}
            </h2>

            {/* Bloc recap destinataire */}
            <div className="theme-bubble-bg p-8 rounded-4xl space-y-8 border theme-border relative overflow-hidden">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <span className="text-sm theme-text-secondary">
                      {t("transfer.verifying_recipient")}
                    </span>
                  </div>
                </div>
              ) : recipientError ? (
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex flex-col gap-1 flex-1">
                    ...
                    <span className="font-bold text-red-500 text-sm">
                      {t("transfer.recipient_not_found")}
                    </span>
                    <span className="text-xs font-bold bg-red-100/60 dark:bg-red-900/20 text-red-500 px-2 py-0.5 rounded-lg w-fit">
                      {keyValue}
                    </span>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="w-12 h-12 rounded-full theme-primary-bg text-white flex items-center justify-center font-bold shadow-lg shrink-0 overflow-hidden">
                    {resolvedRecipient?.avatarUrl ? (
                      <img
                        src={resolvedRecipient.avatarUrl}
                        alt=""
