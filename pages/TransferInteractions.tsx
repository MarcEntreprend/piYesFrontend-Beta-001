...
import { Transaction, TransactionType, TransactionRole, Contact } from '../shared/types';
...
const TransferInteractions: React.FC = () => {
  ...
  const months = [
    t('common.months_full.january'), t('common.months_full.february'), t('common.months_full.march'), 
    t('common.months_full.april'), t('common.months_full.may'), t('common.months_full.june'),
    t('common.months_full.july'), t('common.months_full.august'), t('common.months_full.september'), 
    t('common.months_full.october'), t('common.months_full.november'), t('common.months_full.december')
  ];
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        setTransactions(history.filter(tx => tx.type === TransactionType.TRANSFER));
      } catch (error) {
 ...
 const filteredTransactions = useMemo(() => {
  ...
  }, [transactions, searchTerm, filter, selectedMonth, selectedYear, contactId]);
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            <h1 className="text-xl font-black theme-text-main tracking-tight">
              {contact ? t('reports.labels.interactions.with_contact', { name: contact.name.split(' ')[0] }) : t('reports.labels.interactions.my_interactions')}
            </h1>
            {contact && <p className="text-[10px] theme-text-secondary font-bold uppercase tracking-widest">{contact.tag || contact.phone}</p>}
          </div>
        </div>

        <div className="flex gap-2">
          <div className="relative flex-1">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 theme-text-secondary opacity-50" size={18} />
            <input 
              type="text"
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              placeholder={t('reports.labels.interactions.search_placeholder')}
              className="w-full py-3 pl-10 pr-4 theme-bubble-bg rounded-2xl outline-none theme-text-main border theme-border focus:border-[var(--primary-color)] transition-all text-sm font-bold"
            />
          </div>
          <button 
              ...
              >
                <div className="flex items-center gap-4">
                  <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${
                    tx.role === TransactionRole.PAYER ? 'bg-red-50 text-red-500' : 'bg-green-50 text-green-500'
                  }`}>
                    {tx.role === TransactionRole.PAYER ? <ArrowUpRight size={24} /> : <ArrowDownLeft size={24} />}
                  </div>
                  <div>
                    <h4 className="font-bold theme-text-main text-sm">{tx.counterpartyName}</h4>
                    <p className="text-[10px] theme-text-secondary flex items-center gap-1">
                      <Calendar size={10} /> {new Date(tx.date).toLocaleDateString('fr-HT', { day: 'numeric', month: 'short' })}
                ...
                <div className="text-right">
                  <p className={`font-black text-sm ${
                    tx.role === TransactionRole.PAYER ? 'theme-text-main' : 'text-green-600'
                  }`}>
                    {tx.role === TransactionRole.PAYER ? '-' : '+'} {tx.amount.toLocaleString('fr-HT')} G
                  </p>
                  <ChevronRight size={14} className="theme-text-secondary opacity-30 ml-auto mt-1" />
                </div>
              </motion.div>
            ))}
          </div>
          ...
          <div className="fixed inset-0 z-[100] flex items-end justify-center">
            ...
            >
              <div className="flex justify-between items-center">
                ...
                <button onClick={() => setShowMonthPicker(false)} className="p-2 theme-bubble-bg rounded-full theme-text-secondary">
                  <X size={20} />
                </button>
              </div>
              ...
              <div className="space-y-6">
                ...
                <Button
                  ...
                  variant="primary"
                  fullWidth
                  className="py-5 rounded-[24px] font-black shadow-xl uppercase tracking-widest mt-4"
                >
