...
import { ArrowLeft, Search, ShieldCheck, ShieldX, Camera, ChevronRight, Share2, Download, ExternalLink } from 'lucide-react';
      ...
      <div className="p-6 space-y-6">
        {!result ? (
          <div className="space-y-4 animate-in fade-in">
            <p className="text-sm theme-text-secondary">{t('verification.instructions')}</p>
            <div className="flex gap-2">
              <div className="relative flex-1">
                ...
                <input 
                  ...
                  className="w-full theme-bubble-bg rounded-xl py-4 pl-10 pr-4 theme-text-main outline-none border-2 border-transparent focus:border-[var(--primary-color)] transition-all"
                />
              </div>
              <button 
          ...
          <div className="animate-in zoom-in duration-300 space-y-6">
            <div className="p-6 bg-green-50 dark:bg-green-900/10 rounded-2xl border border-green-100 dark:border-green-900/20">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <div>
                    <h3 className="font-bold text-green-700 dark:text-green-400">{t('verification.authentic_receipt')}</h3>
                    <p className="text-[10px] text-green-600 uppercase tracking-widest font-bold">{t('verification.status_label')} {result.status}</p>
                  </div>
                  <button onClick={() => setResult(null)} className="ml-auto p-1 theme-text-secondary">
                    <XCircle size={20} />
                  </button>
                </div>
                ...
                <div className="space-y-4 border-t border-green-100 dark:border-green-900/20 pt-4">
                  <div className="flex justify-between items-center">
                    <span className="text-xs theme-text-secondary">{t('receipt.amount_label')}</span>
                    <span className="text-sm font-bold theme-text-main">{result.amount.toLocaleString('fr-HT')} {result.currency}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-xs theme-text-secondary">{t('receipt.date_label')}</span>
                    <span className="text-sm font-bold theme-text-main">{new Date(result.date).toLocaleDateString('fr-HT')}</span>
                  </div>
                  
                  {result.sender && (
                    <div className="flex justify-between items-center">
                      <span className="text-xs theme-text-secondary">{t('receipt.sender')}</span>
                      <span className="text-sm font-bold theme-text-main">{result.sender.name}</span>
                    </div>
                  )}
                  {result.receiver && (
                    <div className="flex justify-between items-center">
                      <span className="text-xs theme-text-secondary">{t('receipt.receiver')}</span>
                      <span className="text-sm font-bold theme-text-main">{result.receiver.name}</span>
                    </div>
                  )}
                </div>

                <div className="mt-6 pt-4 border-t border-green-100 dark:border-green-900/20">
                    <p className="text-[10px] font-bold theme-text-secondary uppercase mb-1">{t('receipt.external_id')}</p>
                    <p className="text-[10px] font-mono theme-text-main opacity-70 truncate">{result.transaction_id}</p>
                </div>
            </div>
        ...
        {error && (
          <div className="p-6 bg-red-50 rounded-2xl border border-red-100 flex flex-col items-center text-center animate-in zoom-in">
            ...
            <h3 className="font-bold text-red-700">{t('verification.invalid_id')}</h3>
            <p className="text-xs text-red-600 mt-1">{error}</p>
            <button onClick={() => setError(null)} className="mt-4 text-xs font-bold theme-text-secondary">{t('common.retry')}</button>
          </div>
        )}
      </div>
    </div>
