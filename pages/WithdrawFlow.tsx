...
import { ArrowLeft, CheckCircle2, Wallet, FileText, Home, Landmark, Loader2, HelpCircle, MapPin, Search, ChevronRight, Clock, QrCode, AlertTriangle, X } from 'lucide-react';
...
const WithdrawFlow: React.FC<WithdrawFlowProps> = ({ user, onUpdateUser }) => {
  ...
  const handleExitWarning = useCallback((e: BeforeUnloadEvent) => {
    if (step === 'code') {
      e.preventDefault();
      e.returnValue = '';
    }
...
/>
        ...
        {step === 'amount' && (
          <div className="flex-1 animate-in slide-in-from-right duration-300 flex flex-col">
            <div className="flex-1">
              <h2 className="text-2xl font-bold mb-8 theme-text-main">{t('withdraw.amount_q', { currency: t('currency.name_plural') })}</h2>
              <div className="flex items-center border-b-2 theme-border pb-2 mb-10 focus-within:border-[var(--primary-color)] transition-colors">
        ...
        {step === 'agent' && (
          <div className="flex-1 animate-in slide-in-from-right duration-300 flex flex-col">
            <div className="flex-1 space-y-6">
              <h2 className="text-2xl font-bold theme-text-main">{t('withdraw.where_q')}</h2>
                ...
                {filteredAgents.map(agent => (
                  ...
                  >
                    <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 ${agent.type === 'atm' ? 'bg-blue-500' : 'bg-green-500'} text-white`}>
                      {agent.type === 'atm' ? <Landmark size={24} /> : <MapPin size={24} />}
                    </div>
                    <div className="flex-1">
        ...
        {step === 'code' && (
          <div className="flex-1 animate-in zoom-in duration-300 flex flex-col items-center text-center space-y-8 pt-4">
            <div className="space-y-2">
              ...
              <h2 className="text-2xl font-black theme-text-main">{t('withdraw.present_code')}</h2>
              <p className="theme-text-secondary text-sm px-8">
                <span dangerouslySetInnerHTML={{ __html: t('withdraw.present_instruction', { name: selectedAgent?.name, amount }) }} />
              </p>
            </div>

            <div className="bg-white p-8 rounded-[40px] shadow-2xl border-4 border-gray-50 flex flex-col items-center gap-6 w-full max-w-[300px]">
              ...
              <div className="space-y-1">
                <p className="text-[10px] font-black theme-text-secondary uppercase tracking-[0.2em]">{t('withdraw.alphanumeric_code')}</p>
                <p className="text-4xl font-black theme-primary-text tracking-widest">{withdrawalCode}</p>
              </div>
            </div>

            <div className="flex items-center gap-3 theme-bubble-bg px-6 py-3 rounded-full border theme-border">
              <Clock size={18} className={timeLeft < 60 ? 'text-red-500' : 'theme-primary-text'} />
              <p className={`font-black ${timeLeft < 60 ? 'text-red-500' : 'theme-text-main'}`}>
            ...
            <div className="w-full pt-4">
              <Button
                fullWidth
                onClick={() => setResult({ status: 'success', tx: currentTx })}
              >
      ...
      <Modal isOpen={showExitConfirm} onClose={() => setShowExitConfirm(false)}>
        <div className="p-8 space-y-6 text-center">
          ...
          <div className="space-y-2">
            <h3 className="text-xl font-bold theme-text-main">{t('withdraw.exit_confirm_title')}</h3>
            <p className="text-sm theme-text-secondary">{t('withdraw.exit_confirm_desc')}</p>
          </div>
      ...
      <AiSupportChat isOpen={showSupport} onClose={() => setShowSupport(false)} context={t('actions.withdraw')} />
    </div>
  );
};
