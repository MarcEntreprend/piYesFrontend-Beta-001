...
const AiSupportChat: React.FC<AiSupportChatProps> = ({
  isOpen,
  onClose,
  context,
}) => {
  ...
  const handleSend = async () => {
    ...
    const response = await aiService.getSupportResponse(
      input,
      context,
      language,
    );
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 no-scrollbar bg-white dark:bg-black/20 shadow-inner">
          {messages.map((m) => (
            ...
            >
              ...
              <span className="text-[8px] font-black theme-text-secondary uppercase opacity-50 px-2">
                {m.timestamp}
              </span>
            </div>
          ))}
          {isTyping && (
            <div className="flex flex-col gap-1.5 self-start items-start animate-pulse">
              <div className="p-4 rounded-3xl rounded-bl-none theme-bubble-bg theme-border border shadow-sm">
                <Loader2
                  size={16}
        ...
        <div className="p-6 border-t theme-border shrink-0 bg-gray-50/30 dark:bg-white/5">
          ...
          >
            <input
              ...
              className="flex-1 theme-bubble-bg py-4 pl-6 pr-14 rounded-3xl outline-none theme-text-main text-sm focus:theme-card-bg border theme-border shadow-sm transition-all"
            />
            <button
              type="submit"
