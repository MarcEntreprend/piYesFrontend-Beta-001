...
const AdDetail: React.FC = () => {
  ...
  const ad = {
    ...
    description:
      "État neuf, batterie 100%, 512GB. Facture fournie. Vendu avec coque de protection et verre trempé déjà posé. Pas de rayures, toujours utilisé avec soin.",
    price: 185000,
    ...
    specs: [
      {
        label: t("ad_detail.specs.storage"),
        value: "512GB",
        icon: <HardDrive size={18} />,
      },
      {
        label: t("ad_detail.specs.color"),
        value: "Titane",
        icon: <Palette size={18} />,
      },
      {
        label: t("ad_detail.specs.year"),
        value: "2024",
    ...
    <div className="theme-card-bg min-h-screen pb-32 flex flex-col animate-in slide-in-from-right duration-400">
      <header className="px-6 pt-12 pb-4 fixed top-0 w-full z-50 flex justify-between items-center pointer-events-none">
        ...
        <div className="flex gap-2 pointer-events-auto">
          <button className="p-3 bg-black/30 backdrop-blur-md text-white rounded-full active:scale-90">
            <Share2 size={22} />
          </button>
          <button
            onClick={() => setShowMenu(!showMenu)}
            className="p-3 bg-black/30 backdrop-blur-md text-white rounded-full active:scale-90"
          >
      ...
      <div className="relative aspect-square w-full bg-gray-100 overflow-hidden">
        ...
        <div className="absolute bottom-6 left-6 bg-white/90 backdrop-blur-md px-6 py-3 rounded-3xl shadow-2xl border border-white/20">
          <span className="text-2xl font-black theme-primary-text">
            {ad.price.toLocaleString("fr-HT")} {t("currency.symbol")}
          </span>
        </div>
      </div>
      ...
      <div className="p-8 space-y-8">
        <div className="space-y-2">
          <div className="flex justify-between items-start gap-4">
            <h1 className="text-2xl font-black theme-text-main leading-tight">
              {ad.title}
            </h1>
            <button
              onClick={toggleFavorite}
              className={`p-3 rounded-full ${isFavorite ? "bg-red-50 text-red-500" : "theme-bubble-bg text-secondary"}`}
            >
          ...
          <div className="flex flex-wrap gap-4 items-center">
            ...
            <div className="flex items-center gap-1.5 text-[10px] font-bold theme-text-secondary uppercase tracking-widest bg-gray-100 dark:bg-white/5 px-3 py-1.5 rounded-full">
              ...
              {t("ad_detail.published_on", { date: ad.date })}
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3">
          {ad.specs.map((spec, i) => (
            <div
              key={i}
              className="p-4 theme-bubble-bg border theme-border rounded-3xl flex flex-col items-center gap-2 text-center group"
            >
        ...
        <div className="space-y-3">
          <h3 className="text-[11px] font-black theme-text-secondary uppercase tracking-[0.2em] px-1">
            Description
          </h3>
          <p className="text-sm theme-text-main leading-relaxed opacity-80 bg-gray-50/50 dark:bg-white/5 p-6 rounded-4xl border theme-border">
            {ad.description}
          </p>
        </div>

        <div className="p-6 theme-bubble-bg rounded-[40px] border theme-border flex items-center gap-5">
          <div className="w-16 h-16 rounded-3xl border-4 border-white dark:border-gray-800 shadow-xl overflow-hidden shrink-0">
            <img
          ...
          <div className="flex-1 space-y-0.5">
            <div className="flex items-center gap-2">
              <p className="font-black theme-text-main text-sm">
                {ad.seller.name}
              </p>
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[10px] theme-text-secondary opacity-50">
                • {t("ad_detail.member_since", { year: ad.seller.memberSince })}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
            ...
            {[
              {
                ...
                rating: 5,
                comment:
                  "Excellent produit, conforme à la description. Vendeur très sérieux.",
                date: "Il y a 2 jours",
                verified: true,
              },
              {
              ...
              <div
                key={review.id}
                className="p-6 theme-card-bg border theme-border rounded-4xl space-y-3 shadow-sm"
              >
                <div className="flex justify-between items-start">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[9px] theme-text-secondary font-medium">
                        {review.date}
                      </p>
                    </div>
                  </div>
                ...
                <p className="text-xs theme-text-main leading-relaxed opacity-80">
                  {review.comment}
                </p>
                {review.verified && (
                  <div className="flex items-center gap-1.5 text-[8px] font-black text-green-600 uppercase tracking-widest bg-green-50 dark:bg-green-900/20 px-2 py-1 rounded-full w-fit">
                    <ShieldCheck size={10} />{" "}
                    {t("boutique.reviews.verified_purchase")}
                  </div>
                )}
              </div>
            ))}
          ...
          <button className="w-full py-5 theme-bubble-bg border theme-border rounded-3xl text-[10px] font-black theme-text-secondary uppercase tracking-[0.2em] active:scale-95 transition-all">
        ...
        <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-md theme-card-bg border-t theme-border p-6 pb-10 z-60 flex gap-4">
          <button
            ...
            className="flex-1 flex items-center justify-center gap-3 theme-bubble-bg theme-primary-text py-5 rounded-3xl font-black text-xs active:scale-95 border theme-border"
          >
          ...
          {ad.seller.acceptsPiyes ? (
            <button
              onClick={() =>
                navigate(
                  `/transfer?name=${encodeURIComponent(ad.title)}&amount=${ad.price}`,
                )
              }
              className="flex-1 flex items-center justify-center gap-3 theme-primary-bg text-white py-5 rounded-3xl font-black text-xs active:scale-95 shadow-xl"
            >
            ...
            <button
              ...
              className="flex-1 flex items-center justify-center gap-3 bg-gray-900 text-white py-5 rounded-3xl font-black text-xs active:scale-95 shadow-xl"
            >
