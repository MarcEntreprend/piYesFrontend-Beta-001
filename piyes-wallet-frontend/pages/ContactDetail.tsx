...
const ContactDetail: React.FC<ContactDetailProps> = ({ user }) => {
  ...
  const highlightFriendshipFromUrl =
    new URLSearchParams(search).get("highlight") === "friendship";

  const [contact, setContact] = useState<Contact | null>(null);
  const [friendships, setFriendships] = useState<Friendship[]>([]);
  const [loading, setLoading] = useState(true);
  const [highlightFriendship, setHighlightFriendship] = useState(
  ...
  if (!contact) {
    return (
      <div className="theme-card-bg min-h-screen flex flex-col items-center justify-center gap-4 p-12">
        ...
        <button
          onClick={() => navigate(-1)}
          className="theme-primary-text font-bold"
        >
          Retour
        </button>
      </div>
        ...
        {!isUserContact && (
          <div className="flex items-center gap-3 p-4 bg-red-50 dark:bg-red-900/10 rounded-2xl border border-red-200 dark:border-red-900/30">
            ...
            <p className="text-xs font-bold text-red-500 leading-relaxed">
              Ce contact n'est pas encore un utilisateur piYès. Les actions de
              paiement sont désactivées jusqu'à son inscription.
            </p>
          </div>
        )}
