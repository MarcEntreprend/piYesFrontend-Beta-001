...
const HelpCenter: React.FC = () => {
  ...
  const categories = [
    {
      id: "pix",
      label: t("help_center.categories.pix"),
      icon: <Repeat size={20} />,
    },
    {
      id: "card",
      label: t("help_center.categories.card"),
      icon: <CreditCard size={20} />,
    },
    {
      id: "security",
      label: t("help_center.categories.security"),
      icon: <Shield size={20} />,
    },
    {
      id: "account",
      label: t("help_center.categories.account"),
      ...
      <div className="p-6 space-y-8">
        <div className="relative group">
          ...
          <input
            ...
            className="w-full theme-bubble-bg p-4 pl-12 pr-4 rounded-2xl outline-none theme-text-main border theme-border focus:theme-card-bg focus:border-(--primary-color) transition-all shadow-sm"
          />
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          {categories.map((cat) => (
            <button
              key={cat.id}
              className="p-4 theme-bubble-bg border theme-border rounded-2xl flex flex-col items-center gap-2 text-center group active:scale-95 transition-all shadow-sm"
            >
              <div className="theme-primary-text group-hover:scale-110 transition-transform">
                {cat.icon}
              </div>
              <span className="text-[10px] font-bold theme-text-main uppercase tracking-wider">
                {cat.label}
              </span>
            </button>
          ))}
        ...
        <div className="space-y-4">
          <h3 className="text-xs font-bold theme-text-secondary uppercase tracking-widest px-1">
            {t("help_center.faqs.title")}
          </h3>
            ...
            {faqs.map((faq, i) => (
              ...
              >
                <div className="space-y-1 min-w-0 pr-2">
                  ...
                  <p className="text-[10px] theme-text-secondary italic opacity-70">
                    "{faq.a}"
                  </p>
                </div>
                <ChevronRight
                  size={16}
                  className="theme-text-secondary opacity-30 group-hover:opacity-100 group-hover:translate-x-1 transition-all shrink-0"
                />
              </button>
            ))}
          </div>
